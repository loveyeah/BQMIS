...
Ext.extend(Ext.ux.Radio, Ext.form.Radio, {
			onRender : function(ct, position) {
				...
				if (this.el && this.el.dom) {
					this.el.dom.value = this.value;// make the value for radio
													// is the value user has
													// given!
				}
...
Ext.extend(Ext.ux.RadioGroup, Ext.form.Field, {
			...
			getValue : function() {
				...
				return false;
				/**//*
					 * if(this.value !== undefined){ return this.value; }else
					 * if(this.el.dom.value.length > 0){ return
					 * this.el.dom.value; }
					 */
			},
			setValue : function(v) {
