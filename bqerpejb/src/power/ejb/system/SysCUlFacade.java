...
//				+ enterpriseCode + "' and t.worker_code='" + workerCode
			+ enterpriseCode + "' and (t.login_code='" + workerCode + "' or t.worker_code='" + workerCode + "')"
				+ " and t.login_pwd='" + password + "' and t.is_use='Y' ";
	...
	public boolean checkUserRight(String enterpriseCode, String workerCode,
		...
		String sql = "select count(1) from sys_c_ul t where t.enterprise_code='"
				+ enterpriseCode
				// modified by liuyi 20100407 启动新工号
//				+ "' and t.worker_code='"+ workerCode+ "' " 
				+ "' and (t.worker_code='"+ workerCode+ "' or t.login_code='"+workerCode+"') "
				+" and t.login_pwd='" + password + "' and t.is_use='Y' ";
	...
	public PageObject getUsersBy(String enterpriseCode,
			String workerCodeOrName, int... rowStartIdxAndCount) {
		String sql = "select * from sys_c_ul a where a.enterprise_code='"
			// modified by liuyi 20100406 新工号启用修改查询条件
//				+ enterpriseCode + "' and a.worker_code||a.worker_name like '%"
			+ enterpriseCode + "' and a.login_code||a.worker_name like '%"
				+ workerCodeOrName + "%' and a.is_use='Y' order by a.worker_id";
		String sqlCount = "select count(1) from sys_c_ul a where a.enterprise_code='"
				+ enterpriseCode
				+ "' and a.login_code||a.worker_name like '%"
				+ workerCodeOrName + "%' and a.is_use='Y'";
	...
	public PageObject findUserByDeptAndRole(Long roleId, String fuzzy,
...
//						+ "                 where d.is_use = 'U'\n"
			...
			List<SysCUl> list = bll.queryByNativeSQL(sql,
					new Object[] { roleId }, SysCUl.class, rowStartIdxAndCount);
			Long totalCount = Long.parseLong(bll.getSingal(countsql,
	...
	public String findWorkerCodeByName(String workerName) {
		// modified by liuyi 20100406 启用新工号
//		String sql = "select t.worker_code from sys_c_ul t where t.is_use='Y' and t.worker_name='"
		String sql = "select t.login_code from sys_c_ul t where t.is_use='Y' and t.worker_name='"
				+ workerName + "' and rownum=1";
	...
	public PageObject findWorkerByRoleId(String roleId,String deptCode) {
		PageObject result = new PageObject();
		String sql = "select b.* \n"
				+ "  from sys_j_ur a, sys_c_ul b,Hr_j_Emp_Info d \n" + " where a.role_id = '"
				+ roleId + "'\n"
				+ "   and a.worker_id = b.worker_id\n"
				+ "    and d.emp_code=b.worker_code\n"
		...
		if(deptCode!=null && !"".equals(deptCode)){
			sql+=" and d.dept_id='"+deptCode+"'";
			sqlCount+=" and d.dept_id='"+deptCode+"'";
		}
		List<SysCUl> list = bll.queryByNativeSQL(sql,SysCUl.class);
