		...
		try {
			...
			// else
			{
				sql = "select\n"
						...
						+ "r.role_type,\n"
						+ "r.accredit_section,\n"
						+ "(select t.dept_name from hr_c_dept t where t.dept_id=r.accredit_section) as deptName\n"
			...
			while (it.hasNext()) {
				...
				if (ob[9] != null)
					bmi.setAccreditSection(ob[9].toString());
				if (ob[10] != null)
					bmi.setAccreditSectionname(ob[10].toString());
				arrlist.add(bmi);
			}
