 ...
 */
@Stateless
public class PrincipalAuditeFacade implements PrincipalAuditeFacadeRemote {
    @Resource
    SessionContext ctx;
    /** db操作类接口 */
    ...
    /** 考勤部门表操作接口 */
    @EJB(beanName = "HrCAttendancedepFacade")
    protected HrCAttendancedepFacadeRemote deptManager;
    /** 是否使用: 是 */
    ...
    /** 全局是否已审核（满足设计书中非法数据情况（已审核，审核标准却为空...）） */
    private String IF_CHECKED = BLANK_STRING;
    /** 没有结果抛出错误类型 */
    public static final String NO_RESULT_N = "N";
    /** 日期列dataindex标识符 */
    public static final String DATE_DATAINDEX = "D";
    /** 合计项列dataindex标识符 */
    public static final String TOTAL_DATAINDEX = "C";
    /** 合计项单位天 */
    public static final String UNIT_DAY = "天";
    /** 合计项单位小时 */
    public static final String UNIT_HOUR = "小时";
    /** 未审核部门分隔符 */
    public static final String UNCHECK_CUT = ",";
    /** 日期列开始位置 */
    public static final int DATE_START_LOCAL = 6;
    /** 员工信息列宽度 */
    public static final int WIDTH_EMP_INFO = 100;
    /** 日期列宽度 */
    public static final int WIDTH_DATE = 75;
    /** 合计项列宽度 */
    public static final int WIDTH_TOTAL = 95;
    /** 没有考勤标准的错误 */
    public static final String ERROR_X = "X";
    /** 没有考勤标准的错误 */
    public static final String ERROR_XY= "XY";
    ...
    public TimeKeeperExamineForm getExamineInfo(String examineDate,
            String argDeptId, String argEnterpriseCode, Properties p, String empDeptId)
            throws Exception {
        LogUtil.log("EJB:负责人审核查询开始", Level.INFO, null);
        try {
            ...
            // 查询该父部门下所有子部门的考勤信息
            List<TimeKeeperExamine> arrlist = gridMainList(strYear, strMonth,
                    argEnterpriseCode, argDeptId);
            Long totalCount = 0L;
            if (arrlist != null) {
            ...
            if (arrlist.size() > 0) {
                ...
                if (0 == detpList.size()) {
                	List<HrCAttendancedep> deptList = deptManager.findByAttendanceDeptId(Long.parseLong(argDeptId));
                	if (null != deptList && deptList.size() > 0) {
                		dataAll.setSonCheckFlag(deptList.get(0).getAttendanceDeptName());
                		return dataAll;
                	}
                }
                ...
                if (argDeptId.equals(tempDept)
                    ...
                    IF_CHECKED = FLAG_N;
                    if (null == arrlist.get(0).getExamineMan1()) {
                    	dataAll.setSonCheckFlag(arrlist.get(0)
                              .getExamineBDeptName());
                    	return dataAll;
                    }
                }
                // 设置可撤销FLG = 是
                arrlist.get(0).setCancelFlag(FLAG_Y);
                // 获取该部门该年该月的开始日期结束日期
                ...
                // 查询上面部门里的所有员工的考勤信息
                List<TimeKeeperExamineContent> arrListContent = allEmpAttendanceList(
                        detpList, strStartDate, strEndDate, argEnterpriseCode,
                        p);
                ...
                while (i < recordsCount) {
                    ...
                    if(isEmpty(arrListContent.get(i).getAtendanceDate())){
                        for (int j = 0; j < daysCount; j++) {
                            cell.put(obj.getMetaData().getFields().get(j + DATE_START_LOCAL).get("dataIndex"), BLANK_STRING);
                    ...
                    for (int j = 0; j < daysCount; j++) {
                        ...
                        // 如果有当天的记录
                        if (i < recordsCount && cs.equals(cn) && cell.get("empId").equals(arrListContent.get(i).getEmpId())) {
                            countTotalItem(arrListContent.get(i), cell, totalNameList, standardTime);
                            cell.put(obj.getMetaData().getFields().get(j + DATE_START_LOCAL).get("dataIndex"), arrListContent.get(i)
                                    .getMark());
                            i++;
                        } else {
                            // 没有当天的记录则插入空白
                ...
                LogUtil.log("EJB:负责人审核查询结束。", Level.INFO, null);
            } else {
            	// 没有满足部门id = 画面.部门id的部门，则报错
            	List<HrCAttendancedep> deptList = deptManager.findByAttendanceDeptId(Long.parseLong(argDeptId));
            	if (null != deptList && deptList.size() > 0) {
            		dataAll.setSonCheckFlag(deptList.get(0).getAttendanceDeptName());
            	}
            ...
            LogUtil.log("EJB:负责人审核查询错误。子部门未审核。", Level.SEVERE, e);
            ctx.setRollbackOnly();
            throw e;
        } catch (ParseException e) {
            LogUtil.log("EJB:负责人审核查询错误，日期转换失败。", Level.SEVERE, null);
            ctx.setRollbackOnly();
            throw e;
            ...
            LogUtil.log("EJB:负责人审核查询错误。", Level.SEVERE, null);
            ctx.setRollbackOnly();
            throw e;
        }
    }
...
//                }
            ...
            LogUtil.log("EJB:根据部门id、年份、月份查询部门grid错误", Level.INFO, e);
            throw e;
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public List<TimeKeeperExamineContent> allEmpAttendanceList(
            List<String> deptIds, String strStartDate, String strEndDate,
            String argEnterpriseCode, Properties p) throws Exception {
        ...
        try {
            ...
            sbdContent
                    .append("       ,CONCAT(CONCAT(G.WORK_SHIFT_MARK,E.VACATION_MARK),F.OVERTIME_MARK) AS MARK  ");
            ...
            sbdContent.append("       ,D.LAST_MODIFIY_DATE    ");
            ...
            listParamsContent.add(IS_USE_Y);
            listParamsContent.add(argEnterpriseCode);
            // modify by liuyi 090923 10:11 部门表中用U标识使用中
//            listParamsContent.add(IS_USE_Y);
            //modify by kzhang 20100903
            ...
            while (itContent.hasNext()) {
                ...
                if (null != data[17]) {
                    tempBean.setWorkShitFee(Double.parseDouble(data[17]
                            .toString()));
                }
                if (null != data[18]) {
                    tempBean.setLastModifiyDate(data[18].toString());
                }
                // 设置考勤标志
    ...
    public void saveTotalCountByEmp(List<String> deptIds, String strStartDate,
        ...
        try {
            ...
            // 查询上面部门里的所有员工的考勤信息
            List<TimeKeeperExamineContent> arrListContent = allEmpAttendanceList(
                    deptIds, strStartDate, strEndDate, argEnterpriseCode, p);
            // 定义津贴费用map
            ...
            for (int index = 0; index < arrListContent.size(); index++) {
                ...
                if (!isEmpty(tempBean.getWorkShiftId())) {
                    if (isEmpty(fee.get(tempBean.getWorkShiftId()))) {
                        fee.put(tempBean.getWorkShiftId(), tempBean
                                .getWorkShitFee());
                    }
                }
                ...
                if (isEmpty(mapAll.get(tempBean.getEmpId()))) {
                    // 不存在则new一个该员工的map
                    empMap.put(CACodeConstants.STAT_ITEM_TYPE_OVERTIME,
                            new HashMap<Object, Long>());
                    empMap.put(CACodeConstants.STAT_ITEM_TYPE_RUN,
                            new HashMap<Object, Long>());
                    empMap.put(CACodeConstants.STAT_ITEM_TYPE_VACATIONID,
                            new HashMap<Object, Long>());
                    empMap.put(CACodeConstants.STAT_ITEM_TYPE_WORK,
                            new HashMap<Object, Long>());
                ...
                if (!isEmpty(tempBean.getOvertimeTypeId())) {
                    // 判断该加班id是否已经存在
                    if (isEmpty(empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_OVERTIME).get(
                            tempBean.getOvertimeTypeId()))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_OVERTIME)
                                .put(tempBean.getOvertimeTypeId(), (long) 1);
                    } else {
                        empMap
                                .get(CACodeConstants.STAT_ITEM_TYPE_OVERTIME)
                                .put(
                                        tempBean.getOvertimeTypeId(),
                                        empMap
                                                .get(
                                                        CACodeConstants.STAT_ITEM_TYPE_OVERTIME)
                                                .get(
                                                        tempBean
                                                                .getOvertimeTypeId()) + 1);
                    }
                ...
                if (!isEmpty(tempBean.getWorkShiftId())) {
                    // 判断该运行班id是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_RUN)
                            .get(tempBean.getWorkShiftId()))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_RUN).put(
                                tempBean.getWorkShiftId(), (long) 1);
                    } else {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_RUN).put(
                                tempBean.getWorkShiftId(),
                                empMap.get(CACodeConstants.STAT_ITEM_TYPE_RUN)
                                        .get(tempBean.getWorkShiftId()) + 1);
                    }
                }
                // 判断该员工有无假别id
                if (!isEmpty(tempBean.getVacationTypeId())) {
                    // 判断该运行班id是否已经存在
                    if (isEmpty(empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_VACATIONID).get(
                            tempBean.getVacationTypeId()))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_VACATIONID)
                                .put(tempBean.getVacationTypeId(), (long) 1);
                    } else {
                        empMap
                                .get(CACodeConstants.STAT_ITEM_TYPE_VACATIONID)
                                .put(
                                        tempBean.getVacationTypeId(),
                                        empMap
                                                .get(
                                                        CACodeConstants.STAT_ITEM_TYPE_VACATIONID)
                                                .get(
                                                        tempBean
                                                                .getVacationTypeId()) + 1);
                    }
                ...
                if (FLAG_1.equals(tempBean.getWork())) {
                    // 判断出勤是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_WORK))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                                CACodeConstants.ATTENDANCE_TYPE_WORK, (long) 1);
                    } else {
                        empMap
                                .get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                                .put(
                                        CACodeConstants.ATTENDANCE_TYPE_WORK,
                                        empMap
                                                .get(
                                                        CACodeConstants.STAT_ITEM_TYPE_WORK)
                                                .get(
                ...
                if (FLAG_1.equals(tempBean.getRest())) {
                    // 判断出勤是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_REST))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                                CACodeConstants.ATTENDANCE_TYPE_REST, (long) 1);
                    } else {
                        empMap
                                .get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                                .put(
                ...
                if (FLAG_1.equals(tempBean.getAbsent())) {
                    // 判断旷工是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_ABSENT_WORK))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                ...
                if (FLAG_1.equals(tempBean.getLate())) {
                    // 判断迟到是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_LATE_WORK))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                ...
                if (FLAG_1.equals(tempBean.getLeave())) {
                    // 判断早退是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_LEAVE_EARLY))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                ...
                if (FLAG_1.equals(tempBean.getOut())) {
                    // 判断外借是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_OUT_WORK))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
                ...
                if (FLAG_1.equals(tempBean.getEvection())) {
                    // 判断出差是否已经存在
                    if (isEmpty(empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                            .get(CACodeConstants.ATTENDANCE_TYPE_EVECTION))) {
                        empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK).put(
            ...
            for (Object empId : mapAll.keySet()) {
                ...
                for (Object id : empMap.get(
                        CACodeConstants.STAT_ITEM_TYPE_OVERTIME).keySet()) {
                    // 定义加班bean
                    ...
                    overTimeBeanId.setOvertimeTypeId(Long.parseLong(id
                            .toString()));
                    // 赋值id
                    overTimeBean.setId(overTimeBeanId);
                    // 赋值天数
                    overTimeBean.setDays((double) empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_OVERTIME).get(id));
                    // 设置部门
                    overTimeBean.setDeptId(empMap.get("deptId").get("deptId"));
                    // 设置考勤部门
                    overTimeBean.setAttendanceDeptId(empMap.get("deptId").get("attendanceDeptId"));
                    // 赋值是否使用
                ...
                for (Object id : empMap.get(CACodeConstants.STAT_ITEM_TYPE_RUN)
                        .keySet()) {
                    // 定义运行班bean
                    ...
                    workshiftBeanId.setWorkShiftId(Long
                            .parseLong(id.toString()));
                    // 赋值id
                    workShiftBean.setId(workshiftBeanId);
                    // 赋值天数
                    workShiftBean.setDays((double) empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_RUN).get(id));
                    // 设置部门
                    workShiftBean.setDeptId(empMap.get("deptId").get("deptId"));
                    // 设置金钱
                    workShiftBean.setMoney(fee.get(id)
                            * workShiftBean.getDays());
                    // 赋值是否使用
                ...
                for (Object id : empMap.get(CACodeConstants.STAT_ITEM_TYPE_VACATIONID)
                        .keySet()) {
                    // 定义请假bean
                    ...
                    vacationBeanId.setVacationTypeId(Long.parseLong(id
                            .toString()));
                    // 赋值id
                    vacationBean.setId(vacationBeanId);
                    // 赋值天数
                    vacationBean.setDays((double) empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_VACATIONID).get(id));
                    // 设置部门
                    vacationBean.setDeptId(empMap.get("deptId").get("deptId"));
                    // 赋值是否使用
                ...
                for (Object id : empMap.get(CACodeConstants.STAT_ITEM_TYPE_WORK)
                        .keySet()) {
                    // 定义考勤bean
                    ...
                    workDaysBeanId.setAttendanceTypeId(Long.parseLong(id
                            .toString()));
                    // 赋值id
                    workDaysBean.setId(workDaysBeanId);
                    // 赋值天数
                    workDaysBean.setDays((double) empMap.get(
                            CACodeConstants.STAT_ITEM_TYPE_WORK).get(id));
                    // 设置部门
                    workDaysBean.setDeptId(empMap.get("deptId").get("deptId"));
                    // 赋值是否使用
            ...
            for (int i = 0; i < overtimeTotal.size(); i++) {
                ...
                HrDOvertimetotal tempBean = overTimeTotalManager.findById(
                        newBean.getId(), argEnterpriseCode);
                if (isEmpty(tempBean)) {
            ...
            for (int i = 0; i < workShiftTotal.size(); i++) {
                ...
                HrDWorkshifttotal tempBean = workShiftTotalManager.findById(
                        newBean.getId(), argEnterpriseCode);
                if (isEmpty(tempBean)) {
                    newBean.setLastModifiyDate(new Date());
            ...
            for (int i = 0; i < vacationTotal.size(); i++) {
                ...
                HrDVacationtotal tempBean = vacationTotalManager.findById(
                        newBean.getId(), argEnterpriseCode);
                if (isEmpty(tempBean)) {
                    newBean.setLastModifiyDate(new Date());
            ...
            for (int i = 0; i < workDaysTotal.size(); i++) {
                ...
                HrDWorkdays tempBean = workDaysTotalManager.findById(newBean
                        .getId(), argEnterpriseCode);
                if (isEmpty(tempBean)) {
                    newBean.setLastModifiyDate(new Date());
            ...
            // 查找该id的记录是否存在
            HrJAttendancecheck lastBean = attendanceCheckRemote.findById(
                    newBeanId, argEnterpriseCode);
            if (isEmpty(lastBean)) {
                // 如果不存在，插入新记录
                newBean.setDepCharge2(checkEmpId);
                newBean.setCheckedDate2(new Date());
            	...
            	if(0 == (lastBean.getLastModifiyDate().compareTo(formatStringToDate(lastModifiyDate, TIME_FORMAT)))){
	                // 如果存在，更新记录
	                lastBean.setDepCharge2(checkEmpId);
	                lastBean.setCheckedDate2(new Date());
            ...
            LogUtil.log("EJB:考勤审核错误", Level.SEVERE, e);
            ctx.setRollbackOnly();
            throw e;
    ...
    private AttendanceStandard getStartAndEndDate(String strYear,
        ...
        try {
            ...
            if (lstResult.size() == 1) {
            	...
            	// 判断是否已审核，如是，则两条消息都报，否则只报无考勤标准
            	if (FLAG_N.equals(IF_CHECKED)) {
            		throw new RuntimeException(ERROR_X);
            	} else if (FLAG_Y.equals(IF_CHECKED)){
            		throw new RuntimeException(ERROR_XY);
            	}
            }
    ...
    private MetaData creatGridHeader(String strStartDate, String strEndDate,
        ...
        try {
            ...
            String[] days = { BLANK_STRING, "一", "二", "三", "四", "五", "六", "七", "八", "九",
    ...
    public PageObject getDeptsByTopDeptid(String enterpriseCode, Long loginEmp) {
        ...
        sql.append("    T.ATTEND_CHECKER_ID = ?     ");
        sql.append("ORDER BY        ");
        sql.append("    T.ATTENDANCE_DEPT_ID    ");
        List list = bll.queryByNativeSQL(sql.toString(), new Object[] { IS_USE_Y, FLAG_2, enterpriseCode, loginEmp });
        ...
        while (it.hasNext()) {
         ...
         // add by liuyi 20100203 将该考勤审批部门下各层级的代考勤部门的负责人审核权限取到
            String powerSql = 
    ...
    public TimeKeeperExamineStandardTime getEmpAttendance(String empId,
            ...
            String attendanceDeptId, String deptId, String empDeptId) throws Exception {
        ...
        try {
            ...
            // 获取该员工所在考勤部门的标准时间
            AttendanceStandard timeTemp = getStartAndEndDate(strStartDate
                    .substring(0, 4), strStartDate.substring(5, 7),
                    attendanceDeptId, enterpriseCode, empDeptId);
            // 将标准时间赋值到返回的结果里
            ...
            if (FLAG_Y.equals(checkFlag)) {
                ...
                //
                // 查询该员工的当月的请假信息
                List<TimeKeeperExamineEmpHoliday> holidayList = getEmpHoliday(
                        empId, enterpriseCode, strStartDate.substring(0, 7));
                for (int i = 0, k = 0; i < dateFromTo(strStartDate, strEndDate); i++) {
                    ...
                    if (k < list.size()
                        ...
                        tempBean.setAttendanceDeptId(isEmpty(attendanceDeptId)?
                        		isEmpty(deptId)?null:Long.parseLong(deptId):
                        			Long.parseLong(attendanceDeptId));
                        // 如果上班休息FLG = 休息 1
                        if (workOrRestList.get(i).get("workOrRest").equals(
                                FLAG_1)) {
                            String inHoliday = FLAG_N;
                            // 循环判断是否是假期
                            for (int j = 0; j < holidayList.size(); j++) {
                                // 如果当天的日期在一条假日范围内
                                if (dateCompare(caStartDate, holidayList.get(j)
                                        .getStrStartDate()) >= 0
                                        && dateCompare(caStartDate, holidayList
                                                .get(j).getStrEndDate()) <= 0) {
                                    // 如果包含周末：1
                                    if (FLAG_1.equals(holidayList.get(j)
                                            .getIfWeekEnd())) {
                                        // 设置为假别id
                                        tempBean.setVacationTypeId(Long
                                                .parseLong(holidayList.get(j)
                                                        .getVacationTypeId()));
                                        // 如果不包含周末 0
                                    } else if ("0".equals(holidayList.get(j)
                                            .getIfWeekEnd())) {
                                        // 设置为休息
                            ...
                            // 如果上班休息FLG = 上班 2
                        } else if (workOrRestList.get(i).get("workOrRest")
                                .equals(FLAG_2)) {
                            String inHoliday = FLAG_N;
                            // 循环判断是否是假期
                            for (int j = 0; j < holidayList.size(); j++) {
                            ...
                            if (inHoliday.equals(FLAG_N)) {
                                ...
                                if (checkWorkKind(empId, enterpriseCode)) {
                                    // 则设置标准考勤时间
                                    tempBean.setAmBegingTime(timeTemp
                                            .getAmBegingTime());
                                    tempBean.setAmEndTime(timeTemp
                                            .getAmEndTime());
                                    tempBean.setPmBegingTime(timeTemp
                                            .getPmBegingTime());
                                    tempBean.setPmEndTime(timeTemp
                                            .getPmEndTime());
                                }
                            }
                        }
        ...
        try {
            ...
            for (int i = 0; i < records.size(); i++) {
                ...
                if (map.get("id.empId") != null) {
                    ...
                    // 设置考勤日期
                    workAttendanceId
                            .setAttendanceDate(formatStringToDate((String) map
                                    .get("id.attendanceDate"), DATE_FORMAT));
                    // 根据人员ID和考勤日期查询考勤记录
                    bean = empAttendanceManager.findById(workAttendanceId,
                            enterpriseCode);
                    // 结果为空
                    if (bean == null) {
                ...
                // 设置运行班类别
                if (!BLANK_STRING
                        .equals(objectToString(map.get("workShiftId")))) {
                ...
                // 设置假别ID
                if (!BLANK_STRING.equals(objectToString(map
                        .get("vacationTypeId")))) {
                ...
                // 设置出差
                if (BLANK_STRING
                        .equals(objectToString(map.get("evectionType")))) {
                ...
                if (map.get("id.empId") == null) {
                    ...
                    HrJWorkattendanceId workAttendanceId = new HrJWorkattendanceId();
                    // 设置人员ID
                    workAttendanceId.setEmpId(Long.parseLong(empIdInsert));
                    // 设置考勤日期
                    workAttendanceId
                            .setAttendanceDate(formatStringToDate((String) map
                                    .get("id.attendanceDate"), DATE_FORMAT));
            ...
            LogUtil.log("EJB:批量保存员工考勤记录错误。（程序异常）", Level.SEVERE, null);
            ctx.setRollbackOnly();
            throw e;
        }
    ...
    public void cancelLastCheck(String attendanceDeptId, String checkYear,
            String checkMonth, String enterpriseCode, String workerCode, /** 上次修改时间 */
        ...
        try {
            ...
            checkId.setAttendanceDep(Long.parseLong(attendanceDeptId));
            checkId.setAttendanceYear(checkYear);
            checkId.setAttendanceMonth(checkMonth);
            HrJAttendancecheck bean = attendanceCheckRemote.findById(checkId,
                    enterpriseCode);
            if(0 != (bean.getLastModifiyDate().compareTo(formatStringToDate(lastModifiyDate,
                    TIME_FORMAT)))) {
                throw new DataChangeException(null);
            }
            bean.setCheckedDate1(null);
            bean.setDepCharge1(null);
            bean.setLastModifiyDate(formatStringToDate(lastModifiyDate,
                    TIME_FORMAT));
            ...
            LogUtil.log("EJB:撤销前回审核错误。（db操作失败）", Level.SEVERE, null);
            ctx.setRollbackOnly();
            throw re;
        }
