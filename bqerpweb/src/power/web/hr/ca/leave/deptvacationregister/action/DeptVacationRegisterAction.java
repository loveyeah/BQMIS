 ...
 * All right reserved.
 */
package power.web.hr.ca.leave.deptvacationregister.action;
 ...
 * @version 1.0
 */
public class DeptVacationRegisterAction extends AbstractAction  {
     ...
     * 构造函数
     */
    public DeptVacationRegisterAction() {
		// 请假登记ejb远程对象
     ...
     */
    @SuppressWarnings("unchecked")
	public void getDeptVacationInfo() {
     ...
     * 查询考勤部门名称
     */
    public void getDeptAttendanceDept() {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void getDeptVacationType() {
        try {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void getDeptEmpInfo() {
        try {
            LogUtil.log("Action:部门人员信息查询开始。", Level.INFO, null);
            PageObject obj = new PageObject();
            if (employee.getDeptId() != null) {
                // 查询部门人员信息
                obj = remote.getDeptEmpInfo(employee.getDeptId(), null,employee.getEnterpriseCode());
          		// 追加空格
        		if(obj.getList().size() > 0){
        			DeptClearLeaveEmp info = new DeptClearLeaveEmp();
        			info.setEmpId(null);
        			info.setEmpName("");
        			info.setDeptName("");
        			obj.getList().add(0, info);
	...
	public void calculateDept() {
        try {
            LogUtil.log("Action:计算部门请假时长和天数开始。", Level.INFO, null);
    		EmpVacationRegisterAction empAction = new EmpVacationRegisterAction();
            ...
            write(str);
            LogUtil.log("Action:计算部门请假时长和天数结束。", Level.INFO, null);
        } catch (SQLException sqle) {
            LogUtil.log("Action:计算部门请假时长和天数失败。", Level.SEVERE, sqle);
			write(Constants.SQL_FAILURE);
        } catch (ParseException e) {
            LogUtil.log("Action:计算部门请假时长和天数失败。", Level.SEVERE, e);
			write(Constants.DATA_FAILURE);
		} catch (DataChangeException e) {
			if (EXCEPTION_MESSAGE.equals(e.getMessage())) {
				try {
