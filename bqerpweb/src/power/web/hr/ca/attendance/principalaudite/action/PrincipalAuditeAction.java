 ...
 * All right reserved.
 */
package power.web.hr.ca.attendance.principalaudite.action;
...
import power.ejb.hr.ca.PrincipalAuditeFacadeRemote;
 ...
 * @version 1.0
 */
public class PrincipalAuditeAction extends AbstractAction {
	/**
	 * 查询条件类
	 * @author huangweijie
	 */
    ...
    /** 设置配置文件session名称 */
    public static String SESSION_PROPERTY = "mark";
    /** 审核年月 */
    public String examineDate;
    /** 审核部门 */
    public String examineDept;
    ...
    public String node;
    /** 定义ejb接口 */
    public PrincipalAuditeFacadeRemote remote;
    /** 开始日期 */
    ...
    /** 上次修改时间 */
    public String lastModifiyDate;
    /** 所有部门 */
    public String deptIds;
    /** 错误代码：考勤标准不存在 */
    private static final String ERROR_X = "X";
    /** 错误代码：考勤标准不存在，已审核 */
    private static final String ERROR_XY = "XY";
    /** 合计项的关键字：C */
     ...
     * @throws IOException
     */
    public PrincipalAuditeAction() throws IOException {
        remote = (PrincipalAuditeFacadeRemote) factory
                .getFacadeRemote("PrincipalAuditeFacade");
        // 加载配置文件
        InputStream fileSteam = CaCommonBusinessAction.class
    ...
    public void getAttendanceList() {
        try {
            ...
            obj = remote.getExamineInfo(examineDate, examineDept, employee
                    .getEnterpriseCode(), p, String.valueOf(employee.getDeptId()));
            // 把登录员工的ID和姓名放进去，为考核准备
            obj.setLoginEmpId(String.valueOf(employee.getWorkerId()));
            obj.setLoginEmpName(employee.getWorkerName());
            ...
            // 设置需要导出的查询条件
            request.getSession().setAttribute("PrinQueryObj", qObj);
            ...
            LogUtil.log("Action:考勤合计项维护初始化失败", Level.SEVERE, null);
            ...
            // 没有对应考勤日期
        	if (ERROR_X.equals(msg)) {
	            LogUtil.log("EJB:没有对应的考勤日期。", Level.SEVERE, null);
	            write("{success:true,msg:'" + ERROR_X + "'}");
        	} else if (ERROR_XY.equals(msg)){
                // 且已审核
        		LogUtil.log("EJB:没有对应的考勤日期，且记录已审核。", Level.SEVERE, null);
	            write("{success:true,msg:'" + ERROR_XY + "'}");
        	} else {
        		LogUtil.log("Action:考勤合计项维护初始化失败", Level.SEVERE, null);
                write(Constants.DATA_FAILURE);
        	}
    ...
    public void getEmpAttendanceInfo() {
        try {
            ...
            obj = remote.getEmpAttendance(empId, employee.getEnterpriseCode(),
                    strStartDate, strEndDate, checkFlag, workOrRestList,
                    attendanceDeptId, deptId, String.valueOf(employee.getDeptId()));
            String str = JSONUtil.serialize(obj);
            write(str);
    ...
    public void saveEmpAttendanceInfo() {
        try {
            ...
            ArrayList<Map<String, Object>> empAttendanceList = (ArrayList<Map<String, Object>>) JSONUtil
                    .deserialize(records);
            // 查询员工的考勤记录
            List<String> deptAllIds = (ArrayList<String>) JSONUtil
                    .deserialize(deptIds);
            // 保存员工考勤信息
            remote.saveEmpAttendance(empAttendanceList, employee
                    .getEnterpriseCode(), employee.getWorkerCode(), empId,
                    strStartDate, strEndDate, deptAllIds, p);
            write("{success:true,msg:'S'}");
    ...
    public void setAttendanceCheck() {
        try {
            ...
            List<String> deptAllIds = (ArrayList<String>) JSONUtil
                    .deserialize(deptIds);
            // 撤销审核
            remote.saveTotalCountByEmp(deptAllIds, strStartDate, strEndDate,
                    employee.getEnterpriseCode(), p, employee.getWorkerCode(),
                    attendanceDeptId, employee.getWorkerId(), lastModifiyDate);
            // 成功返回
            write("{success:true,msg:'S'}");
    ...
    public void cancelLastCheck() {
        try {
            ...
            // 撤销审核
            remote.cancelLastCheck(attendanceDeptId, checkYear, checkMonth,
    ...
    public void exportAttendanceQueryFile() throws Exception {
        try {
            ...
            QueryObj qObj = (QueryObj) request
                    .getSession().getAttribute("PrinQueryObj");
