...
Ext.onReady(function(){
    ...
    function chooseWorker(){
    	var selectNodes = runGrid.getSelectionModel().getSelections();
			if (selectNodes.length == 0) {
				Ext.Msg.alert("提示", "请选择工作票!");
				return false;
			}
			var ros = new Array();
			var workticketNos = new Array();
			//var workerNames = new Array();
			//var deptIds = new Array();
			for (var i = 0; i < selectNodes.length; i++) {
				var record = selectNodes[i];
				workticketNos.push(record.get('model.workticketNo'));
    ...
    var runGrid = new Ext.grid.GridPanel({
        ...
        tbar : ['时间范围：', planStartDate, '~', planEndDate,
            	...
            	{xtype : "tbseparator"}, queryBtn,confirmBtn],
        //分页
    ...
    function returnValues() {
        if (runGrid.selModel.hasSelection()) {
			...
			if (selected.length == 0) {
				...
				alert(member.get('model.workticketNo'))
