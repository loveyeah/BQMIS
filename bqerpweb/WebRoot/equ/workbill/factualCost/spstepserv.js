...
Ext.onReady(function() {
	...
	function addRecords() {
		...
		var o = new datalist({
			...
			'baseInfo.factServiceCode' : '',
			'baseInfo.factServiceUnit' : '',
			'baseInfo.factFee' : '',
			'factServName' : ''
		...
		if (modifyRec.length > 0 || ids.length > 0) {
			Ext.Msg.confirm("提示信息", '确定要保存修改数据吗?', function(button) {
				if (button == 'yes') {
					...
					Ext.Ajax.request({
								url : 'workbill/saveEquJStandardService.action',
								method : 'post',
								params : {
			...
			editor : new Ext.form.TextField({
				...
				listeners : {
					focus : function(e) {
						...
						var rvo = window
								.showModalDialog(
										'/power/equ/standardpackage/maint/equCStandardOrderstep/service.jsp',
										args,
						...
						if (typeof(rvo) != "undefined") {
							...
							record.set("factServName", rvo.name);
							record.set("baseInfo.factServiceCode",
									rvo.code);
//									rvo.serUnit

							// document.getElementById("workerCode").value =
							// rvo.workerCode;
							// document.getElementById("user.workerCode").value
							// =
							// rvo.workerCode;
