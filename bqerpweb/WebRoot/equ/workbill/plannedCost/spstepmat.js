...
Ext.onReady(function() {
	...
	if (stdopCode != '' ) {
		...
		url4 = 'workbill/getEquCMainmat.action?woCode=' + stdWoCode + '&opCode='
				+ stdopCode;
		...
		columns : [centerGridsm, new Ext.grid.RowNumberer(), {
				...
				listeners : {
					focus : function(e) {
						...
						if (typeof(rvo) != "undefined") {
							...
							record.set("baseInfo.planMaterialCode",
									rvo.workerCode);
							record.set("matName", rvo.workerName);
			...
			editor : new Ext.form.ComboBox({
						...
						store : mystore2 = new Ext.data.SimpleStore({
									fields : ['planLocationName',
											'planLocationId'],
									data : planLaborCode_data
								}),

						displayField : 'planLocationName',
						valueField : 'planLocationId',
						mode : 'local',
						value : '',
						readOnly : true,
						triggerAction : 'all',
			...
			renderer : function changeIt(val) {
				for (i = 0; i < mystore2.getCount(); i++) {
					if (mystore2.getAt(i).get("planLocationId") == val)
						return mystore2.getAt(i).get("planLocationName");
				}
			...
			renderer : function(value, params, record) {
				var v = (10000
						* Number(record.get("baseInfo.planMaterialPrice")) * Number(record
