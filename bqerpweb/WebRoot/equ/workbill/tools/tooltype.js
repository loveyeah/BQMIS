...
Ext.onReady(function() {
	...
	function initForm() {
		Ext.get("tooltype.Id").dom.value = "";
		Ext.get("tooltype.TypeName").dom.value = "";
		Ext.get("tooltype.markCode").dom.value = "";

		Ext.get("tooltype.memo").dom.value = "";
	}
	// ---------树的click事件
	function clickTree(node) {
		...
		Ext.Ajax.request({
					url : 'workbill/findInfoById.action',
					params : {
						node : currentNode.id
					},
					method : 'post',
					...
					success : function(result, request) {
						...
						Ext.get("tooltype.id").dom.value = json.id;
						Ext.get("tooltype.pid").dom.value = (json.pid == null)?"":json.pid;
						Ext.get("tooltype.typename").dom.value = json.typename;
						Ext.get("tooltype.markCode").dom.value = json.markCode;
						...
						if (json.memo == null) {
							...
							Ext.get("tooltype.memo").dom.value = json.memo;
						}
						// if (json.isUse == 'Y') {
						// check();
						// } else {
	...
	var prjTypeName = {
		id : "tooltype.typename",
		xtype : "textfield",
		allowBlank : false,
		fieldLabel : '类别名称',
			listeners:{
					change:function(){ 
						if(Ext.get("tooltype.typename").dom.value =="")
						return ; 
						var arrRslt = makePy(Ext.get("tooltype.typename").dom.value);
						Ext.get("tooltype.markCode").dom.value = arrRslt;
					}
				},
		name : 'tooltype.typename'
	}

	var markCode = {
		id : "tooltype.markCode",
		xtype : "textfield",
		fieldLabel : '识别码',
		allowBlank : false,
		name : 'tooltype.markCode'
	}
			...
			buttons : [{
					...
					Ext.Msg.confirm('提示', '删除后您将不能恢复!确定要删除吗?', function(
							response) {
										if (response == 'yes') {
											myaddpanel.getForm().submit({
												method : 'post',
												url : 'workbill/deleteToolType.action',
												// waitMsg : '正在删除数据,请等待...',
						...
						handler : function() {
							...
							var url = ""
							if (currentType != "add") {
								url = "workbill/updateToolType.action"
							} else
								url = "workbill/addToolType.action"
							if (myaddpanel.getForm().isValid()) {
								myaddpanel.getForm().submit({
									method : 'POST',
									url : url,
...
//											alert(action.response.responseText)
											...
											if (currentNode == null) {
												...
												_node
														.setText(Ext
																.get("tooltype.TypeName").dom.value);
	...
	function add() {
		...
		currentType = "add";
		Ext.get("tooltype.pId").dom.value = currentNode.id;
		Ext.get("PPrjTypeName").dom.value = currentNode.text;
		Ext.get("tooltype.TypeName").dom.readOnly = false;
		Ext.get("tooltype.markCode").dom.readOnly = false;
	...
	Ext.get("tooltype.TypeName").on('click', function() {
				if (currentNode == null) {
	...
	Ext.get("tooltype.markCode").on('click', function() {
				if (currentNode == null) {
					Ext.Msg.alert("提示", "请选择节点在增加");
					return false
				}
