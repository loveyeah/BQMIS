...
Ext.onReady(function() {
	...
	var ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'equstandard/findAll.action'
		}),
	...
	, {
			...
			listeners : {
				focus : function(e) {
					...
					if (typeof(rvo) != "undefined") {
						...
						record.set("baseInfo.planToolCode", rvo.code);
					//rvo.serUnit	
		...
		header : '工具编码',
		dataIndex : 'baseInfo.planToolCode',
		align : 'left',
		hidden : true
			// ,
			// width : auto
						...
						"render" : function() {
							this.el.on("dblclick", function() {
								var record = grid.getSelectionModel()
										.getSelected();
								var value = record.get('baseInfo.planToolDescription');
		...
		if (modifyRec.length > 0 || ids.length > 0) {
			Ext.Msg.confirm('提示信息', '确定要保存修改数据吗?', function(button) {
				if (button == 'yes') {
					...
					Ext.Ajax.request({
								url : 'equstandard/addEquCStandardTools.action',
								method : 'post',
								params : {
