		...
		Ext.Ajax.request({
					...
					success : function(result, request) {
						...
						for (var i = 0; i < actions.length; i++) {
							if (actions[i].changeBusiStateTo == "YCL"
									&& day < 3) {
								// if (day > 3) {
								// actions[i].actionName = "退回";
								// }
								trueActions.push(actions[i]);
							} else if ((day > 3 && day <= 7)
									&& (actions[i].changeBusiStateTo == "TY1")) {
								trueActions.push(actions[i]);
							} else if ((day > 7)
									&& (actions[i].changeBusiStateTo == "TY2")) {
								trueActions.push(actions[i]);
							}
						}
						radioAddHandler(trueActions);
					},
					failure : function() {
					}
				});
				...
				listeners : {
					select : {
						fn : function(combo, record, index) {
							var plcode = record.get('retrunValue');
							if (plcode == "2") {
								delayTime.setDisabled(true);
								delayTime.setValue("");
								btnNextDisplay.setDisabled(true);
							} else {
	...
	var awaitField = new Ext.form.FieldSet({
		...
		items : [{
					...
					items : [{
						...
						items : [{
									xtype : "textarea",
									fieldLabel : "延期意见<font color='red'>*</font>",
					...
					buttons : [{
						...
						handler : function() {
							if (failureApproveFormPanel.getForm().isValid()) {
								if (nrs == ""
										&& (day >= 7 && resultComboBox
												.getValue() == '1')) {
									Ext.Msg
											.alert('提示',
													'请选择下一步角色，发电部管辖设备（主机、机、炉、电等）选择发电部审批，运营公司管辖设备选择运营公司审批！');
									return
								} else {
									failureApproveFormPanel.getForm().submit({
										url : 'bqfailure/repaireAwaitFailure.action?',
										params : {
											actionId : (resultComboBox
													.getValue() == "1")
													? actionId
													: 105,
											nextRoles : nrs,
		...
		success : function(result, request) {
			...
			if (responseArray.success == true) {
				...
				if (hisInfo.data == null || hisInfo.data == "") {
					...
					//alert("a:"+applyDate+"d:"+delayDate+"day:"+day);
				}
