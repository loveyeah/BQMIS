	...
	function getCurrentSteps() {
		Ext.Ajax.request({
			...
			success : function(result, request) { 
				...
				{
					if((day >3 && day <=7) && (actions[i].changeBusiStateTo=="YCL"))
					{
						trueActions.push(actions[i]);
					}
					else if((day >7) && (actions[i].changeBusiStateTo=="TY"))
					{
						trueActions.push(actions[i]);
					}
				}
				...
				handler : function() {
					if (failureApproveFormPanel.getForm().isValid()) {
						failureApproveFormPanel.getForm().submit({
							url : 'bqfailure/depawaitFailure.action?',
							params : {
								actionId : (resultComboBox.getValue() == "1")? actionId : 115,
								nextRoles : nrs,
