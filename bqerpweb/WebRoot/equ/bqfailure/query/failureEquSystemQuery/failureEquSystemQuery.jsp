<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<html>
<head>
<title>缺陷统计报表(按系统)</title>
<jsp:include page="../../../../comm/jsp/commjsp.jsp"></jsp:include>
	<script type="text/javascript" src="comm/ext/ext-lang-zh_CN.js"></script>
	<script type="text/javascript" src="comm/ext/urlparams.js"></script>
	<script type="text/javascript" src="comm/ext/GroupHeaderPlugin.js"></script>
	<link rel="stylesheet" type="text/css"
			href="comm/ext/GroupHeaderPlugin.css" />
	<script type="text/javascript"
			src="comm/datepicker/WdatePicker.js" defer="defer"></script>
			
</head>
<body>
    <script type="text/javascript" src="comm/scripts/Constants.js"></script>
	<script type="text/javascript" src="equ/bqfailure/query/failureEquSystemQuery/failureEquSystemQuery.js"></script>
	<div id="div_lay"></div>
</html>