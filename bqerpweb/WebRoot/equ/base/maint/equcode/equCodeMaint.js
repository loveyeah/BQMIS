	...
	//创建增加和修改窗口
		...
		var win = new Ext.Window({
			...
			buttons : [{
				...
				mypanel.form.submit({
						method : 'POST',
						url : 'equ/saveEquCode.action?method=' + op,
...
//增加学位信息
	function addRecord() {
			if (!myaddpanel) {
			var myaddpanel = createupdatepanel("增加设备编码","自动生成");
		}
		if (!win) {
	...
	function updateRecord() {
			...
			if (recordslen > 1) {
				...
				if (!myaddpanel) {
					var myaddpanel = createupdatepanel("修改设备编码",record.get("id"));
