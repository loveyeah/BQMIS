...
//                    alwaysUseStartDate : false,
    ...
    var storeDept = new Ext.data.JsonStore({
        root : 'list',
        url : "ca/getAttendanceDeptType2InfoPrin.action",
    ...
    var btnExport = new Ext.Button({
        ...
        handler : function() {
            Ext.MessageBox.confirm(Constants.CONFIRM, Constants.COM_C_007, function(button, text) {
                if (button == "yes") {
                    document.all.blankFrame.src = "ca/exportAttendanceQueryFilePrin.action";
                }
            })
        }
    });
    ...
    var storeEmpAttendance = new Ext.data.JsonStore({
        root : 'list',
        url : 'ca/getEmpAttendanceInfoPrin.action',
    ...
    gridMain.on('rowdblclick', function() {
        ...
        if (gridMain.selModel.hasSelection()) {
            ...
            for (i = 0; i < storeCheck.getCount(); i++) {
                ...
                if (r.data['examineADept'] == record.data['attendanceDeptId']) {
                    if (r.data['man2ChsName'] != null && r.data['man2ChsName'] != '') {
                        ...
                        txtCheckMan.setValue(loginEmpName);
                        txtCheckDate.setValue(new Date().format('Y-m-d'));
                    }
                    break;
                } else {
                    continue;
            ...
            if (null == txtCheckMan.getValue() || "" == txtCheckMan.getValue()) {
                txtCheckMan.setValue(loginEmpName);
                txtCheckDate.setValue(new Date().format('Y-m-d'));
            }
            // 已审核保存按钮不可用
...
//                        colspan : 6,
    ...
    grid.on('afteredit', function(object) {
        if ((storeEmpAttendance.getCount() > 0)
  			&& object.field != 'amBegingTime'
  			&& object.field != 'amEndTime'
  			&& object.field != 'pmBegingTime'
    ...
    var btnAttendExamineWin = new Ext.Button({
        ...
        iconCls : Constants.CLS_DEPT_CHECK,
        handler : function() {
            	if (undefined != gridAttendExamineWin.getSelectionModel().getSelected()) {
	                	var rec = gridAttendExamineWin.getSelectionModel().getSelected();
	                	var lastModifiyDate = rec.data['lastModifiyDate'];
            	}else{
            		if (storeCheckAllRight.getCount() > 0) {
            			var rec = storeCheckAllRight.getAt(0);
	                	var lastModifiyDate = rec.data['lastModifiyDate'];
            		}
            	}
            Ext.Msg.confirm(Constants.CONFIRM, Constants.COM_C_011, function(buttonobj) {
                if (buttonobj == 'yes') {
                    ...
                    for (i = 0; i < storeCheck.getCount(); i++) {
                        ...
                        deptIds.push(record.data['examineADept']);
                    }
                    Ext.Ajax.request({
                        url : 'ca/setAttendanceCheckPrin.action',
                        ...
                        success : function(result, request) {
                            if (result.responseText) {
                                ...
                                var succ = o.msg;
                                if (succ == 'S') {
                                    Ext.Msg.alert(Constants.REMIND, Constants.COM_I_026, function(){
                                    	examineWin.hide();
                                    });
    ...
    var btnCancelExamineWin = new Ext.Button({
        ...
        handler : function() {
            if (gridAttendExamineWin.selModel.hasSelection()) {
                var record = gridAttendExamineWin.getSelectionModel().getSelected();
                if (record.data['cancelFlag'] == 'Y') {
                    Ext.Msg.confirm(Constants.CONFIRM, Constants.COM_C_012, function(buttonobj) {
                        if (buttonobj == 'yes') {
                            Ext.Ajax.request({
                                url : 'ca/cancelLastCheckPrin.action',
                                method : Constants.POST,
                                params : {
                                    attendanceDeptId : record.data['examineADept'],
                                    checkYear : record.data['attendanceYear'],
                                    checkMonth : record.data['attendanceMonth'],
                                    lastModifiyDate : record.data['lastModifiyDate']
                                },
                                success : function(result, request) {
                                    if (result.responseText) {
                                        var o = eval("(" + result.responseText + ")");
                                        var succ = o.msg;
                                        if (succ == 'S') {
                                            Ext.Msg.alert(Constants.REMIND, Constants.COM_I_027, function(){
	                                            examineWin.hide();
	                                            cls(true);
                                            });
                                        } else if (succ == Constants.DATE_FAILURE) {
        ...
        Ext.Msg.confirm(Constants.NOTICE_CONFIRM, Constants.COM_C_001, function(buttonobj) {
            if (buttonobj == 'yes') {
                if (checkGridTime()) {
                    ...
                    Ext.Ajax.request({
                        url : 'ca/saveEmpAttendanceInfoPrin.action',
                        method : Constants.POST,
                        params : {
                            records : records,
                            empId : empId,
    ...
    function queryAll(flag) {
        ...
        if (msg == '') {
            ...
            if (!compareDateStr(examineDate.getValue(), new Date().format("Y-m"))) {
                Ext.Ajax.request({
                    url : 'ca/getAttendanceListPrin.action',
                    method : Constants.POST,
                    params : {
                    ...
                    success : function(action) {
                        ...
                        if (o.msg == Constants.DATE_FAILURE) {
                        	...
                        	// 无考勤标准，且已审核
                        	Ext.Msg.alert(Constants.REMIND, Constants.KQ006_I_001, function(){
                            	Ext.Msg.alert(Constants.REMIND, Constants.COM_I_025);
                        	});
//                            btnExamineQuery.setDisabled(false);
                            return;
                         // modify by liuyi 20100203 action中未写N到前台
//                        } else if (o.msg == 'N') {
                            } else if (o.msg == 'U') {
                            Ext.Msg.alert(Constants.REMIND, Constants.COM_I_003);
                            return;
                        } else if (o.msg != undefined || o.msg != null) {
                            var dept = [];
                            btnExamineQuery.setDisabled(false);
                            dept = o.msg.split(",");
                            var msgc = "";
                            for (i = 0; i < dept.length; i++) {
                                msgc += String.format(Constants.KQ007_I_001, dept[i]);
                            }
                            Ext.Msg.alert(Constants.REMIND, msgc);
                            return;
                        } else if (o.sonCheckFlag != undefined) {
                        	// 子部门未审核时，报出该部门，并让审核查询button可用，查询出其子部门的审核信息
                            var dept = [];
                            btnExamineQuery.setDisabled(false);
                            dept = o.sonCheckFlag.split(",");
                            var msgc = "";
                            for (i = 0; i < dept.length; i++) {
                                msgc += String.format(Constants.KQ007_I_001, dept[i]);
                            }
    ...
    function loadMainData(obj, flag) {
        ...
        color = obj.strColor;
        loginEmpId = obj.loginEmpId;
        loginEmpName = obj.loginEmpName;
        storeCheckAllRight.removeAll();
        sonCheckFlag = obj.sonCheckFlag;
        if (obj.store != null) {
        ...
        if (storeMain.getCount() > 0 && storeCheck.getCount() > 0) {
        	...
        	msgc += String.format(Constants.KQ007_I_001, sonCheckFlag);
            Ext.Msg.alert(Constants.REMIND, msgc);
            controlButton(false);
