...
Ext.onReady(function() {
    ...
    // 员工姓名store
    var dsEmp = new Ext.data.JsonStore({
        url : 'ca/getDeptEmpInfo.action',
        root : 'list',
        totalProperty : 'totalCount',
        fields : ['empId', 'empName', 'deptName'],
        listeners : {
    ...
    dsEmp.load();
    dsEmp.on("load", function() {
        if (dsEmp.getCount() >= 1) {
            deptName = dsEmp.getAt(1).get("deptName");
            txtTreeDept.setValue(deptName);
            txtTreeDeptName.setValue(deptName);
        }
    })
    // 员工姓名
    var cmbEmp = new Ext.form.ComboBox({
        width : 100,
        id : "cmbEmp",
        forceSelection : true,
        triggerAction : 'all',
    ...
    var tbarGrid = new Ext.Toolbar({
        border : false,
        height : 25,
        items : ["员工姓名:", cmbEmp, '-', '所属部门:', txtTreeDept, '-', '请假开始日:', txtVacationStart, '~', txtVacationEnd, '-',
    ...
    var dsVacation = new Ext.data.JsonStore({
        url : 'ca/getDeptVacationInfo.action',
        root : 'list',
        totalProperty : 'totalCount',
        fields : dataVacation,
        listeners : {
    ...
    // 员工姓名store
    var dsEmpName = new Ext.data.JsonStore({
        url : 'ca/getDeptEmpInfo.action',
        root : 'list',
        totalProperty : 'totalCount',
        fields : ['empId', 'empName', 'deptName'],
    ...
    dsEmpName.load();
    dsEmpName.on("load", function() {
        hdnEmpId.setValue(cmbEmpName.getValue());
    })
    cmbEmpName.on("select", function() {
        // 新增的场合
        if (newFlag) {
            // 人员id不为空的场合
            if (cmbEmpName.getValue()) {
                // 设值
                setAttendDept(cmbEmpName.getValue());
                // 开始时间
                ...
                // 得到本年度的上，下班时间
                Ext.Ajax.request({
                    url : 'ca/getDeptOnDutyTime.action',
                    method : Constants.POST,
                    params : {
                        empId : cmbEmpName.getValue()
                    },
                ...
                oldEmpName = cmbEmpName.getValue();
                hdnEmpId.setValue(cmbEmpName.getValue());
                txtTreeDeptName.setValue(deptName);
                hdnDeptId.setValue(deptId);
                // 设置控件可用
                controlUseHandler();
                // 人员id为空的场合
            } else {
    ...
    var dsVacationType = new Ext.data.JsonStore({
        url : 'ca/getDeptVacationType.action',
        root : 'list',
        ...
        Ext.Msg.confirm(Constants.CONFIRM, Constants.COM_C_001, function(button) {
            if (button == 'yes') {
                ...
                formPanel.getForm().submit({
                    method : Constants.POST,
                    url : "ca/saveDeptVacation.action",
                    params : {
                        newFlag : newFlag,
                        flag : false,
                        ifWeekend : ifWeekend
    ...
    function setSaveData(value) {
        // 人员id
        cmbEmpName.setValue(value.empId);
        hdnEmpId.setValue(cmbEmpName.getValue());
    ...
    function setAttendDept(value) {
        Ext.Ajax.request({
            url : 'ca/getDeptAttendanceDept.action',
            method : Constants.POST,
            params : {
                empId : value
            },
    ...
    function setValue() {
        ...
        if (record) {
            // 人员姓名
            cmbEmpName.setValue(record.get("empId"));
            // 人员id
    ...
    function isFormChange() {
        ...
        if (originalData == null) {
            formPanel.getForm().items.each(function(f) {
                var data = f.getValue();
                if (f == txtTreeDeptName || f == hdnDeptId) {
                    flag = false;
                } else if (data) {
                    flag = true;
                }
