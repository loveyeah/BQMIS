...
Ext.onReady(function() {
    ...
    var dsVacation = new Ext.data.JsonStore({
        url : 'ca/getPersonalVacationInfo.action',
        root : 'list',
    ...
    var dsHoliday = new Ext.data.JsonStore({
        url : 'ca/getPersonalHoliday.action',
        root : 'list',
    ...
    dsHoliday.on("load", function() {
        ...
        // 得到本年度的上，下班时间
        Ext.Ajax.request({
            url : 'ca/getPersonalOnDutyTime.action',
    ...
    var dsVacationType = new Ext.data.JsonStore({
        url : 'ca/getPersonalVacationType.action',
        root : 'list',
    ...
    var dsEmp = new Ext.data.JsonStore({
        url : 'ca/getPersonalEmpInfo.action',
        root : 'list',
        totalProperty : 'totalCount',
    ...
    dsEmp.on("load", function() {
        if (dsEmp.getCount() >= 1) {
            deptName = dsEmp.getAt(0).get("deptName");
            empName = dsEmp.getAt(0).get("empName");
            txtTreeDept.setValue(deptName);
            txtTreeDeptName.setValue(deptName);
            txtEmp.setValue(empName);
            txtEmpName.setValue(empName);
            // 设置考勤部门名称
            setAttendDept(dsEmp.getAt(0).get("empId"));
        }
    })
    // 开始时间
    ...
    tabPanel.on("tabchange", function() {
        if (tabPanel.getActiveTab() == registerPanel && originalData == null) {
        ...
        Ext.Msg.confirm(Constants.CONFIRM, Constants.COM_C_001, function(button) {
            if (button == 'yes') {
                ...
                formPanel.getForm().submit({
                    method : Constants.POST,
                    url : "ca/savePersonalVacation.action",
                    params : {
                        newFlag : newFlag,
                        flag : false,
                        ifWeekend : ifWeekend
    ...
    function setAttendDept(value) {
        Ext.Ajax.request({
            url : 'ca/getPersonalAttendanceDept.action',
            method : Constants.POST,
            params : {
                empId : value
            },
    ...
    function isFormChange() {
        ...
        if (originalData == null) {
            formPanel.getForm().items.each(function(f) {
                var data = f.getValue();
                if (f == txtEmpName || f == txtTreeDeptName || f == hdnDeptId || f == hdnEmpId) {
                    flag = false;
                } else if (f == txtStartTime && data == startSysDate) {
                    flag = false;
                } else if (f == txtEndTime && data == endSysDate) {
                    flag = false;
                } else if (f == txtAttendDeptName || f == hdnAttendDeptId) {
                    flag = false;
                } else if (data) {
                    flag = true;
                }
    ...
    function calculateHandler() {
        ...
        Ext.Ajax.request({
            url : 'ca/calculatePersonal.action',
            method : Constants.POST,
            params : {
                empId : hdnEmpId.getValue(),
