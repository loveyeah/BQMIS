...
Ext.onReady(function() {
	...
	function getDeptCode(id) {
		...
		conn.open("POST", 'hr/initRewardGrantDept.action?deptId='
				+ workerDeptId, false);
		conn.send(null);
	...
	function getWorkCode() {
		...
		if (conn.status == "200") {
			...
			if (result.workerCode) {
				...
				// add by yliu 20100819
				var conn = Ext.lib.Ajax.getConnectionObject().conn;
				conn.open("POST", 'hr/getRoleIDByWorkerId.action', false);
				conn.send(null);
				if (conn.status == "200") {
					var roleList = eval("(" + conn.responseText + ")");
					if(roleList.length) {
						for(var i=0;i<roleList.length;i++) {
							if(roleList[i] == '168' || roleList[i] == '199') {
//								groupName.setValue(workerDeptId);
//								groupName.setRawValue(result.deptName);
...
//								continue;
	...
	function getPeopleNum() {
		var conn = Ext.lib.Ajax.getConnectionObject().conn;
		conn.open("POST", 'hr/getDeptPeopleNum.action?deptId='+ deptId, false);
		conn.send(null);

		// 成功状态码为200
		if (conn.status == "200") {
			var surePeopleNum = eval("(" + conn.responseText + ")");
	...
	function monthInit() {
		...
		var conn = Ext.lib.Ajax.getConnectionObject().conn;
		conn.open("POST", 'hr/getMaxGarntMonth.action', false);
		conn.send(null);
