...
//		}
	...
	function radioAddHandler() {
		Ext.Ajax.request({
			url : "hr/isHasShfit.action",
			method : 'post',
			params : {
				deptId : deptId
			},
			success : function(result, request) {
				var count = eval('(' + result.responseText + ')');
				var radio = new Array();
				if(count.msg == 0) {
				 	radio = [{actionId:1,actionName:'同意',checked : true},{actionId:2,actionName:'有班组，奖金通知单分发到班组'}];
				} else {
					radio = [{actionId:1,actionName:'同意'},{actionId:2,actionName:'有班组，奖金通知单分发到班组',checked : true}];
				} 
				...
				Ext.lib.Ajax.request('POST',
						'managecontract/contractApproveCheckUser.action', {
							success : function(action) {
								...
								if (result) {
									Ext.Ajax.request({
										url : 'hr/bigRewardAdministratorApprove.action',
										waitMsg : '正在保存数据...',
										method : 'post',
										params : {
											detailId : detailId,
