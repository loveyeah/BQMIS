...
Ext.onReady(function() {
	...
	var dataProxy = new Ext.data.HttpProxy({
		url : 'com/findBasisSalaryList.action'
	});
	...
	var grid = new Ext.grid.GridPanel({
		...
		columns : [sm, new Ext.grid.RowNumberer({
			...
			width : 75,
			sortable : true,
			dataIndex : 'basisSalaryId',
			hidden : true
		}, {
			header : "基础工资",
			width : 75,
			sortable : true,
			dataIndex : 'basisSalary'
		}, {
			header : "执行开始时间",
			width : 75,
			sortable : true,
	...
	var basisSalaryId = new Ext.form.Hidden({
		id : "basisSalaryId",
		fieldLabel : 'ID',
		width : wd,
		readOnly : true,
		value : '',
		name : 'basis.basisSalaryId'
	});

	var basisSalary = new Ext.form.NumberField({
		id : "basisSalary",
		fieldLabel : '基础工资',
		width : wd,
		name : 'basis.basisSalary'
	});
	...
	var myaddpanel = new Ext.FormPanel({
		...
		closeAction : 'hide',
		title : '增加/修改基础工资维护',
		items : [basisSalaryId, basisSalary, effectStartTime, effectEndTime,
				memo]
	});

	function checkInput() {
		var msg = "";
		if (basisSalary.getValue() == "") {
			msg = "'基础工资'";
		}
		if (effectStartTime.getValue() == "") {
			if (msg == "")
		...
		if (selected.length == 0) {
			...
			for (var i = 0; i < selected.length; i += 1) {
				...
				if (member.status == 0) {
					...
					return;
				} else if (member.basisSalaryId) {
					ids.push(member.basisSalaryId);
				} else {
