...
Ext.onReady(function() {
	...
	Ext.ns("OvertimeSalary.overtimeSalaryBase");
	OvertimeSalary.overtimeSalaryBase = function() {
	...
	var MyRecord = Ext.data.Record.create([{
		name : 'overtimeSalaryBaseId',
		mapping : 0
	}, {
		name : 'overtimeSalaryBase',
		mapping : 1
	}, {
	...
	var dataProxy = new Ext.data.HttpProxy({
		url : 'com/findOverTimeSalaryBaseList.action'
	});
	...
	var grid = new Ext.grid.GridPanel({
		...
		columns : [sm, new Ext.grid.RowNumberer({
			...
			width : 75,
			sortable : true,
			dataIndex : 'overtimeSalaryBaseId',
			hidden : true
		}, {
			header : "加班工资基数(元)",
			width : 75,
			sortable : true,
			dataIndex : 'overtimeSalaryBase'
		}, {
			header : "执行开始时间",
			width : 75,
			sortable : true,
	...
	var overtimeSalaryBaseId = new Ext.form.Hidden({
		id : "overtimeSalaryBaseId",
		fieldLabel : 'ID',
		width : wd,
		readOnly : true,
		value : '',
		name : 'base.overtimeSalaryBaseId'
	});

	var overtimeSalaryBase = new Ext.form.NumberField({
		id : "overtimeSalaryBase",
		fieldLabel : '加班工资基数(元)',
		width : wd,
		name : 'base.overtimeSalaryBase'
	});
	...
	var myaddpanel = new Ext.FormPanel({
		...
		closeAction : 'hide',
		title : '增加/修改基础工资维护',
		items : [overtimeSalaryBaseId, overtimeSalaryBase, effectStartTime, effectEndTime,
				memo]
	});

	function checkInput() {
		var msg = "";
		if (overtimeSalaryBase.getValue() == "") {
			msg = "'加班工资基数'";
		}
		if (effectStartTime.getValue() == "") {
			if (msg == "")
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var myurl = "";
				if (overtimeSalaryBaseId.getValue() == "") {
					myurl = "com/addOvertimeSalaryBase.action";
				} else {
					myurl = "com/updateOvertimeSalaryBase.action";
				}
				if (!checkInput())
					return;
	...
	function addRecord() {
		...
		myaddpanel.setTitle("增加加班工资信息");
	}

	function updateRecord() {
		if (grid.selModel.hasSelection()) {
			var records = grid.getSelectionModel().getSelections();
			id = records[0].data.overtimeSalaryBaseId;
		...
		if (selected.length == 0) {
			...
			for (var i = 0; i < selected.length; i += 1) {
				...
				if (member.status == 0) {
					...
					return;
				} else if (member.overtimeSalaryBaseId) {
					ids.push(member.overtimeSalaryBaseId);
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选记录？", function(buttonobj) {
				if (buttonobj == "yes") {
					Ext.lib.Ajax.request('POST',
							'com/deleteOvertimeSalaryBase.action', {
