...
Ext.onReady(function() {
	...
	var dataProxy = new Ext.data.HttpProxy({
		url : 'manageplan/getMonthAward.action'
	});
	...
	var monthAwardId = new Ext.form.Hidden({
		id : "monthAwardId",
		fieldLabel : 'ID',
		width : wd,
		readOnly : true,
		value : '',
		name : 'award.monthAwardId'
	});
	...
	var myaddpanel = new Ext.FormPanel({
		...
		closeAction : 'hide',
		title : '增加/修改月奖基数维护',
		items : [monthAwardId,status, monthAward, effectStartTime, effectEndTime,
				memo]
	});

	function checkInput() {
		var msg = "";
		if (monthAward.getValue() == "") {
			msg = "'月奖基数'";
		}
		if (effectStartTime.getValue() == "") {
			if (msg == "")
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			iconCls : 'save',
			handler : function() {
				var myurl = "manageplan/saveMonthAward.action";
				/*if (basisSalaryId.getValue() == "") {
	...
	function updateRecord() {
		...
		if (grid.selModel.hasSelection()) {
			...
			id = records[0].data.monthAwardId;
			//alert(id)
		...
		if (selected.length == 0) {
			...
			for (var i = 0; i < selected.length; i += 1) {
				...
				if (member.status == 0) {
					...
					return;
				} else if (member.monthAwardId) {
					ids.push(member.monthAwardId);
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选记录？", function(buttonobj) {
				if (buttonobj == "yes") {
					Ext.lib.Ajax.request('POST',
							'manageplan/delMonthAward.action', {
