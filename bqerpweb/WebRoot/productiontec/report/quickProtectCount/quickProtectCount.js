...
//	};
	...
	var ds = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'productionrec/getQuickProtectCountList.action'
						}),
				reader : new Ext.data.JsonReader({
	...
	function checkTime1() {
		var date = this.value;
		Grid.getSelectionModel().getSelected().set("exitDateString", date);
	}
	function checkTime2() {
		var date = this.value;
		Grid.getSelectionModel().getSelected().set("resumeDateString", date);
	}
	...
	function addRec() {
		...
		var ob = new item({
					...
					'protectTypeName' : null,
					'exitDateString' : null,
					'resumeDateString' : null,
	...
	function saveRec() {
		...
		Ext.Msg.confirm('提示信息', '确定要保存修改吗？', function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
							url : 'productionrec/saveQuickProtectCountMod.action',
							method : 'post',
							params : {
	...
	function exportRecord() {
		...
		Ext.Msg.confirm('提示', '确认要导出数据？', function(id) {
			if (id == 'yes') {
				var tableHeader = "<table border=1><tr><th colspan = 7>快速保护（线路高频保护及元件差动保护）统计报表（每月5日前上报）</th></tr>";			
				...
				html
						.push("<tr><th colspan = 3>报表单位："+ dept.getValue()+"</th>"
								+ "<th colspan = 2></td><th colspan = 2>月份："
								+ time + "</th></tr>")
				html
						.push('<tr><th>序号</th><th>被保护设备</th><th>保护型号</th><th>保护种类</th><th>退出时间</th><th>恢复时间</th><th>退出原因及采取措施</th></tr>')
				for (var i = 0; i < ds.getTotalCount(); i++) {
					var rec = ds.getAt(i);			
					html.push('<tr><td>'+(i + 1)+'</td><td>' + rec.get('protectEqu') + '</td><td>' 
							+ rec.get('protectDevice') + '</td><td>'
							+ rec.get('protectType') + '</td><td>'
							+ rec.get('exitDateString') + '</td><td>'
							+ rec.get('resumeDateString') + '</td><td>'
							+ rec.get('exitReason') + '</td>'
							+ '</tr>')
				}
				html.push("<tr><td colspan = 1></td><td colspan = 2>批准:"+ds.getAt(0).get('approveName')+"</td>" +
