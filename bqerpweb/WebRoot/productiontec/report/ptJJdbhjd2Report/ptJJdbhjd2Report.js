...
Ext.onReady(function(){
	...
	var ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'productionrec/getPtJJdbhjd2List.action'
		}),
		...
		Ext.Msg.confirm('提示', '确定要保存吗？', function(button) {
					if (button == 'yes') {
						...
						for (var i = 0; i < ds.getTotalCount(); i++) {
							if (ds.getAt(i).get('jdbhjd2Id') != null) {
						...
						Ext.Ajax.request({
									url : 'productionrec/savePtJjdbhjd2Mod.action',
									method : 'post',
									params : {
	...
	function exportRecord() {
		...
		Ext.Msg.confirm('提示', '确认要导出数据？', function(id) {
			if (id == 'yes') {
				...
				if(tableFlag == '2')
				{
					tableHeader = "<table border=1><tr><th colspan = 8>大唐集团继电保护监督报表2</th></tr>";
					tableHeader += "<tr><th colspan = 8>升压站继电保护及安全自动装置校验计划完成情况月报表</th></tr>"
				}
				else
				{
					tableHeader = "<table border=1><tr><th colspan = 8>大唐集团继电保护监督报表4</th></tr>";
					tableHeader += "<tr><th colspan = 8>升压站继电保护及安全自动装置动作情况月报表</th></tr>"
				}
				var html = [tableHeader];		
				html
						.push("<tr><th colspan = 3>企业名称:大唐灞桥热电厂</th><th colspan = 2></td>"
								+ "<th colspan = 3>报表时间："
								+ ds.getAt(0).get('entryDateString') + "</th></tr>")
				html
						.push('<tr><th></th><th>35KV</th><th>110KV</th><th>220KV</th><th>330KV</th><th>母差保护</th><th>安全自动装置</th><th>其他</th></tr>')
				for (var i = 0; i < ds.getTotalCount(); i++) {
					var rec = ds.getAt(i);
					html.push('<tr><td>' + rec.get('typeName') + '</td><td>' 
							+ rec.get('kv35Num') + '</td><td>'
							+ rec.get('kv110Num') + '</td><td>'
							+ rec.get('kv220Num') + '</td><td>'
							+ rec.get('kv330Num') + '</td><td>'
							+ rec.get('mcProtectNum') + '</td><td>'
							+ rec.get('safeDeviceNum') + '</td><td>'
							+ rec.get('otherNum') + '</td>'
							+ '</tr>')
				}
				html.push('<tr><td colspan = 2>总工程师：'+ds.getAt(0).get('engineerName')+'</td>' +
						'<td></td><td colspan = 2>设备部：'+ds.getAt(0).get('equDeptName')+'</td>' +
						'<td></td><td colspan = 2>填表：'+ds.getAt(0).get('entryByName')+'</td></tr>')
				html.push('</table>');
