	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				...
				var filePath=Ext.get("annex").dom.value;
				filePath=filePath.substring(0,filePath.lastIndexOf('.')+1)+filePath.substring(filePath.lastIndexOf('.')+1,filePath.length).toLowerCase();
		...
		if (records.length == 0) {
			...
			for (var i = 0; i < records.length; i += 1) {
				...
				if (member.get("dependId")) {
					...
					store.remove(westgrids.getAt(i));
				}
			}
