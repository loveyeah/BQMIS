...
Ext.onReady(function() {
	...
	var dateMonth= new Ext.form.TextField({
		name : 'dateMonth',
		value : exdate,
		id : 'dateMonth',
		fieldLabel : "月份",
...
var MyRecord = Ext.data.Record.create([
	...
	var dataProxy = new Ext.data.HttpProxy(
			...
			{
				url:'productionrec/findEnergAnalyseList.action'
			}
...
//---------------------------------------
	...
	var analyseId = new Ext.form.Hidden({
		id : "analyseId",
		fieldLabel : 'ID',
		anchor : "95%",
		readOnly : true,
		value:'',
		name : 'analyse.analyseId'
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var myurl="";
				if (analyseId.getValue() == "") {
					myurl="productionrec/addEnergAnalyseInfo.action";
				} else {
						myurl="productionrec/updateEnergAnalyseInfo.action";
				}
	...
	function updateRecord()
	{
			...
			if (recordslen > 1) {
		        ...
		        month.setValue(record.get("month").substring(0,7));
	...
	function deleteRecord()
	{
		...
		if (records.length == 0) {
			...
			for (var i = 0; i < records.length; i += 1) {
				...
				if (member.get("analyseId")) {
					ids.push(member.get("analyseId")); 
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选的记录？", function(
				...
				if (buttonobj == "yes") {
					...
					Ext.lib.Ajax.request('POST',
							'productionrec/deleteEnergAnalyseInfo.action', {
