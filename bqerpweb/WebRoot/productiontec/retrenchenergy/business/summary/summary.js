...
var MyRecord = Ext.data.Record.create([
	...
	var dataProxy = new Ext.data.HttpProxy(
			...
			{
				url:'productionrec/findEnergySummaryList.action'
			}
...
//---------------------------------------
	...
	var summaryId = new Ext.form.Hidden({
		id : "summaryId",
		fieldLabel : 'ID',
		anchor : "95%",
		readOnly : true,
		value:'',
		name : 'summary.summaryId'
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var myurl="";
				if (summaryId.getValue() == "") {
					myurl="productionrec/addEnergySummaryInfo.action";
				} else {
						myurl="productionrec/updateEnergySummaryInfo.action";
				}
	...
	function updateRecord()
	{
			...
			if (recordslen > 1) {
		        ...
		        year.setValue(record.get("year").substring(0,4));
	...
	function deleteRecord()
	{
		...
		if (records.length == 0) {
			...
			for (var i = 0; i < records.length; i += 1) {
				...
				if (member.get("summaryId")) {
					ids.push(member.get("summaryId")); 
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选的记录？", function(
				...
				if (buttonobj == "yes") {
					...
					Ext.lib.Ajax.request('POST',
							'productionrec/deleteEnergySummaryInfo.action', {
