...
Ext.onReady(function() {
	...
	function radioAddHandler(flg) {
		...
		var _radio = new Ext.form.Radio({
					...
					name : 'rb-approve',
					inputValue : flg ? 24 : 211,
					listeners : {
						'check' : function() {
							var _actionId = flg ? 24 : 211;
							actionId = _actionId;
						}
					},
					checked : true
...
//								}
				...
				items : [titleField, tableField, showAppointNextRoles,
							...
							success : function(action) {
								...
								if (result) {
									...
									params.push("&flowCode=" + flowCode);
									 params.push("&eventIdentify=SB");
									  params.push("&nextRoles="+nrs);

									Ext.lib.Ajax
											.request(
													'POST',
													'productionrec/protectReport.action',
													{
														success : function(
																action) {
															var result = eval("("
	...
	function getFirstSteps() {
		var flg = true;
		if (protectionType == '3'||protectionType == '4') {
			flg = false;
		}
		radioAddHandler(flg);
