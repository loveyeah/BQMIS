	...
	var grid = new Ext.grid.GridPanel({
		...
		tbar : ["机组编码:", apartCodeBox,'-'," 泄露开始时间:", beginDate, "~", endDate, {
			...
			handler : function() {
				...
				parent.document.all.iframe2.src = "productiontec/chemistry/business/condenserLeak/condenserLeakBase.jsp";
	...
	function updateRecord() {
		if (grid.selModel.hasSelection()) {
			...
			if (recordslen > 1) {
				...
				var url = "productiontec/chemistry/business/condenserLeak/condenserLeakBase.jsp";
	...
	function deleteRecord() {
		...
		if (selected.length == 0) {
			...
			for (var i = 0; i < selected.length; i += 1) {
				...
				if (member.get("condenser.nqjxlId")) {
					ids.push(member.get("condenser.nqjxlId"));
				...
				Ext.Msg.confirm("删除", "是否确定删除所选的数据？", function(buttonobj) {
					if (buttonobj == "yes") {
						Ext.lib.Ajax.request('POST',
								'productionrec/deleteCondenserLeak.action', {
