	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var myurl="";
				if (jdzjId.getValue() == "") {
					myurl="productionrec/addPtJJdzjInfo.action";
				} else {
					myurl="productionrec/updatePtJJdzjInfo.action";
				}
	...
	function deleteRecord()
	{
		...
		if (records.length == 0) {
			...
			for (var i = 0; i < records.length; i += 1) {
				...
				if (member.get("model.jdzjId")) {
					ids.push(member.get("model.jdzjId")); 
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选的记录？", function(
				...
				if (buttonobj == "yes") {
					...
					Ext.lib.Ajax.request('POST',
							'productionrec/deletePtJJdzjInfo.action', {
								success : function(action) {
