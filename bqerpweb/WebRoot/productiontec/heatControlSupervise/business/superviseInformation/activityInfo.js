	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var myurl="";
				if (jdhdId.getValue() == "") {
					myurl="productionrec/addPtJJdhdjlInfo.action";
				} else {
					myurl="productionrec/updatePtJJdhdjlInfo.action";
				}
	...
	function deleteRecord()
	{
		...
		if (records.length == 0) {
			...
			for (var i = 0; i < records.length; i += 1) {
				...
				if (member.get("model.jdhdId")) {
					ids.push(member.get("model.jdhdId")); 
				} else {
			...
			Ext.Msg.confirm("删除", "是否确定删除所选的记录？", function(
				...
				if (buttonobj == "yes") {
					...
					Ext.lib.Ajax.request('POST',
							'productionrec/deletePtJJdhdjlInfo.action', {
								success : function(action) {
