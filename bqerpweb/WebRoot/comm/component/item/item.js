Ext.namespace('Power.item');
Power.item = function(config,isShowOne,cmoboConfig){
	// 统计指标
	// 指标时间类型 1时指标，3日指标，4月指标，5季指标，6年指标
	var dataTimeType = typeof(config.dataTimeType) == 'undefined' ? '' : config.dataTimeType
	// 指标数据类型
	var itemType = typeof(config.itemType) == 'undefined' ? '' : config.itemType;
	// isShowOne 是否只传回一个指标
	...
	var con_ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'comm/getAllItemToCommon.action'
		}),
	...
	var deptTree = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			dataUrl : 'comm/findItemTreeToCommon.action'
		}),
		root : rootNodeObj,
		autoWidth : true,
		layout : 'fit',
	...
	deptTree.on('beforeload', function(node) {
		deptTree.loader.dataUrl = 'comm/findItemTreeToCommon.action?pid=' + node.id + '&dataTimeType='
			+ dataTimeType + '&itemType=' + itemType;
	});
