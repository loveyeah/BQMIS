...
Ext.onReady(function(){  
	...
	var rootNode = (arg&&arg.rootNode)?arg.rootNode:{id:'0',text:'灞桥热电厂'};  
	var Tree = Ext.tree;
...
//			}
			...
			{  
				...
				// add by liuyi 20100507 是否班组
				var isBanzus = new Array();
			    if(selectNodes.length >0)
			    {
			    	for(var i=0;i<selectNodes.length;i++)
			    	{
			    		...
			    		isBanzus.push(selectNodes[i].attributes.href);
			    	} 
			    	window.returnValue = {ids:aids.join(","),codes:acodes.join(","),names:anames.join(","),isBanzus:isBanzus.join(",")};
			    	window.close();
			    }
			}
		} ,{
	...
	var wtree = new Tree.TreePanel({
		...
		loader : new Tree.TreeLoader({
			dataUrl : 'comm/getDeptsByPid.action?flag='+flag,
			baseAttrs : {
	...
	wtree.render();
	wroot.expand();
	wtree.on("click",function(node,e){
		 e.stopEvent();
    });
});