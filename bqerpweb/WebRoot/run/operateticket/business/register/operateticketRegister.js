...
var isauto = "N"
var tabUrls = {
	tabReport : 'run/operateticket/business/register/report/opticketReport.jsp',
	tabBaseInfo : '/power/run/operateticket/business/register/baseInfo/opticketBaseInfo.jsp',
...
Ext.onReady(function() {
	...
	var tabPanel = new Ext.TabPanel({
		...
		items : [{
			...
			listeners : {
				activate : function() {
					if (document.tabBaseInfo.operateCode != currentOpCode
							|| currentOpCode == '') {
						document.tabBaseInfo.location = tabUrls.tabBaseInfo;
					}
				}
			},
			html : "<iframe name='tabBaseInfo' src=" + tabUrls.tabBaseInfo
			...
			listeners : {
				activate : function() {
					if (currentOpCode != ""
							&& document.tabContent.operateCode != currentOpCode) {
			...
			html : "<iframe name='tabContent' src=" + tabUrls.tabContent
					+ " style='width:100%;height:100%;border:0px;'></iframe>"
		}, {
			id : 'tabDanger',
			title : '危险点信息列表',
			listeners : {
				activate : function() {
					if (currentOpCode != ""
							&& document.tabDanger.operateCode != currentOpCode) {
			...
			html : "<iframe name='tabDanger' src=" + tabUrls.tabDanger
					+ "  style='width:100%;height:100%;border:0px;'></iframe>"
		}, {
			id : 'tabReport',
			title : '上报列表',
			html : "<iframe name='tabReport' src=" + tabUrls.tabReport
					+ " style='width:100%;height:100%;border:0px;'></iframe>"
		}]
	});
