...
Ext.onReady(function() {
	...
	// // 设置新增操作票的监听器
	// register.addRecord = clearAllFields;
	// // 设置通过操作票编号加载工作票记录
	// register.addLoadOpticketHandler(editOpticket);
				...
				if (typeof(returnValue) != "undefined") {
					...
					Ext.Ajax.request({
						...
						success : function(result, request) {
							...
							opticketType
									.setValue(getOpType(record.data.opticketType))
		...
		baseInfoFormPanel.getForm().submit({
			...
			success : function(form, action) {
				...
				if (isAdd) {
					...
					endop.setVisible(false);
					cbxOpticketSpecials.setDisabled(true);
					// register.opticketCode = o.opticketCode;
				} else {
