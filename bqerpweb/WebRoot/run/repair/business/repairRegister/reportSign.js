...
Ext.onReady(function() {
	...
	function radioAddHandler(flag) {
		...
		var _radio = new Ext.form.Radio({
			...
			name : 'rb-approve',
			inputValue : flag ? 24 : 26,
			listeners : {
				'check' : function() {
					var _actionId = flag ? 24 : 26;
					actionId = _actionId;
				}
			},
			checked : true
		...
		// 'check' : function(){
		// var _actionId =
		// Ext.ComponentMgr.get('approve-radio'+radio[0].actionId).getGroupValue();
									...
									if (Ext.get("showAppointNextRoles").dom.innerHTML == "") {
										Ext.Ajax.request({
											...
											success : function(result, request) {
												...
												form.getForm().submit({
													url : 'manageplan/reportRepairRecord.action',
													waitMsg : '正在保存数据...',
													method : 'post',
													params : {
														repairId : approveId,
	...
	function getFirstSteps() {
		var flag = true;
		if (situationProject == 'N') {
			flag = false;
		}
		radioAddHandler(flag);
	};
