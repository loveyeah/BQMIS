...
Ext.onReady(function() {
	...
	var protectOutNo = new Ext.form.TriggerField({
		fieldLabel : '选择退出申请单编号',
		id : 'protectOutNo',
		// labelSeparator : '',
		xtype : 'textfield',
		name : 'protectOutNo',
		...
		onTriggerClick : function(e) {
			// if (id == null && id == "") {
			var url = "proApplyInlist.jsp";
			var obj = window.showModalDialog(url, null,
					'dialogWidth=700px;dialogHeight=500px;status=no');
			if (typeof(obj) != "undefined") {
				protectOutNo.setValue(obj.protectNo);
				Ext.getCmp('protectName').setValue(obj.protectName);
				Ext.getCmp('hideEquCode').setValue(obj.equCode);
				Ext.getCmp('equCode').setValue(obj.equName);
			}
			// }
		}
	})
	...
	var protectReason = {
		...
		height : 80,
		xtype : "textarea",
		fieldLabel : '投入原因<font color="red">*</font>',
	...
	var applyEndDate = new Ext.form.TextField({
		...
		listeners : {
			...
			// ,
			// validator : function() {
			// var sd = applyStartDate.getValue();
			// var ed = applyEndDate.getValue();
			// if (sd >= ed) {
			// return "申请完工时间应大于开工时间！";
			// } else {
			// return true;
			// }
			// }
	...
	var baseInfoField = new Ext.form.FieldSet({
		...
		border : false,
		buttonAlign : 'center',
		items : [applyId, protectNo, protectOutNo, equCode, hideEquCode, {
			layout : "column",
			border : false,
			items : [{
				columnWidth : 0.5,
	...
	function savePower() {
		...
		if (Ext.get("protectNo").dom.value == "自动生成") {
			url = "protect/addProtectApply.action?isIn=Y"
		} else {
				...
				if (reNo == null || reNo == "") {
					...
					Ext.Ajax.request({
						...
						success : function(result, request) {
							...
							Ext.getCmp('protectOutNo')
									.setValue(currentRecord.data.relativeNo);
							Ext.getCmp('applyBy')
	...
	if (currentRecord != null) {
		...
		Ext.getCmp('protectOutNo').setValue(currentRecord
				.get("power.relativeNo"));
