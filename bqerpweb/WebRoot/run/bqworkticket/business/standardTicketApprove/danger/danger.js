...
//			}
	...
	var grid = new Ext.grid.GridPanel({
		...
		view : new Ext.grid.GroupingView({
			...
			groupTextTpl : '<input type="radio"  value="{[values.rs[0].data.dangerId]}" name="selectOne"></input>{text}'
		})
	}); 
	...
	var dangerpanel = new Ext.FormPanel({
		...
		buttons : [{
			...
			handler : function() {
				// 尚未保存
				if (deleteDangerControl.length > 0 || controlStore.getModifiedRecords().length > 0) {
	...
	function saveRecord() { 
		...
		if (record.length > 0  ||  deleteDangerControl.length>0) {
			for (var i = 0; i < record.length; i++) {
				...
				if(rec.dangerName==null || rec.dangerName==""){
