...
Ext.onReady(function() {
    ...
    var label = new Ext.form.Label({
       ...
       var printDangerButton  = new Ext.Button({
				...
				handler : function()
				{
					...
					var url = '/powerrpt/report/webfile/bqmis/workticketDangerControl.jsp?workticketNo='+strBusiNo;
    ...
    var printPanel = new Ext.Toolbar({ 
		hidden:true, 
    	items:[
    	  printDangerButton,'-',btnPrintWorkticket
    	] 
    });
				...
				if(mytitle == "消防监护人填写消防部门应采取的安措")
				{
					try {
						if (!tabSafety.hasXiaFangSafety()) {
							Ext.Msg.alert("提示", "请填写消防部门应采取的安措！");
							return;
						}
						//没有查看安措
					} catch (err) {
...
//         }
		...
		items : [noticeField,titleField, tableField, showAppointNextRoles, approveSet,
				remarkSet]
	});
...
//				jsonArgs : "{'isDBP':false,'is12Failure':true,'isYSZC':true}"
    ...
    // 票面浏览panel
     ...
     var tab4Html = "<iframe name='tabReport' src='" + tab4Href + "'style='width:100%;height:100%;border:0px;'></iframe>";
   ...
   //----------安措信息控制-------------------------------------   
     ...
     else
     {
       //动火票	
     	...
     	else if(busiStatus=="18") showSafetyTyep="fire"; //消防监护人填写消防安措
...
//    }
   ...
   //-----------------------------------------------
	...
	if (((workticketType == "1" || workticketType == "3" || workticketType == "5") && busiStatus == "6")  ||   ((workticketType == "2" ||workticketType == "7" || workticketType == "8") && busiStatus == "4")||(workticketType=="4"&&busiStatus=="27"&&fireLevelId=="1")) {
	...
	{
		if(busiStatus=="2"||busiStatus=="3"||busiStatus=="7") noticeField.setVisible(false);
		else noticeField.setVisible(true);
	...
	else if(workticketType=="4")
	{
		if(fireLevelId==1)
		{
			...
			if(busiStatus=="3"||busiStatus=="20"||busiStatus=="29"||busiStatus=="24"||busiStatus=="26") noticeField.setVisible(true);
			else noticeField.setVisible(false);
		}
		if(fireLevelId==2)
		{
			//工作许可人、班长审核、领导批准、工作许可人、值班负责人
			if(busiStatus=="3"||busiStatus=="20"||busiStatus=="11"||busiStatus=="24"||busiStatus=="26") noticeField.setVisible(true);
		    else noticeField.setVisible(false);
		}
