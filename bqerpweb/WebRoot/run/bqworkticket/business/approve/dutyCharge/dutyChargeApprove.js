...
Ext.onReady(function() {
    ...
    var label = new Ext.form.Label({
        id : "label",
        text : mytitle,
         style : "color:blue;font-size:25px;line-height:50px;padding-left:180px;font-weight:bold"
    });
    // 标题
 ...
 //----------------------
       var approveFinishDate = new Ext.form.TextField({
        id : 'approveFinishDate',
        fieldLabel : "批准工作结束时间",
        name : 'approveFinishDate',
        style : 'cursor:pointer',
        anchor : "90%",
        value : getDate(),
        listeners : {
 ...
 //----------------------   
    ...
    var form = new Ext.FormPanel({
		...
		items : [noticeField,titleField, finishDatePanel, tableField, showAppointNextRoles,
                ...
                Ext.lib.Ajax.request('POST', 'comm/workticketApproveCheckUser.action',
                    {success : function(action) {
                        ...
                        if (result) {
                           //----------审批------------------------
            form.getForm().submit({
			url : 'bqworkticket/dutyChargeApprove.action',
			waitMsg : '正在保存数据...',
			method : 'post',
			params : {
				...
				approveFinishDate:approveFinishDate.getValue()
			},
...
//				jsonArgs : "{'isDBP':false,'is12Failure':true,'isYSZC':true}"
    ...
    var ticketPanel = new Ext.Panel({
     ...
     var tab2Href="run/bqworkticket/business/approve/safety/workticketSafety.jsp?workticketNo="+strBusiNo+"&workticketType="+workticketType+"&showSafetyTyep=Y";
     ...
     function initData() {
		       Ext.lib.Ajax.request('POST',
				'bqworkticket/getWorkticketBaseInfoByNo.action', {
					...
					var plandate=record.data.model.planEndDate;
					plandate=plandate.substring(0, 10)+" " +plandate.substring(11,21);
					approveFinishDate.setValue(plandate);
