...
Ext.onReady(function() {
        ...
        handler:function(){
         ...
         Ext.Ajax.request({
				...
				success : function(result, request) {
					...
					if(content.opticketNos == null ||content.opticketNos == '')
					{
			        ...
			        {
			        	workticketMemo.setValue(workticketMemo.getValue()+content.opticketNos);
			        }
				},
  ...
  //工作票主表的备注  
    ...
    var memoPanel = new Ext.Panel({
        ...
        // labelWidth:40,
        style : "padding-top:10;border-width:1px 0 0 0;",
       // items:[memoLabel,btnOpticketSelect,workticketMemo]
        items : [{
					columnWidth : 0.1,
					layout : "form",
					border : false,
    ...
    var form = new Ext.FormPanel({
		...
		items : [noticeField,titleField, tableField, showAppointNextRoles, approveSet,
				memoPanel,memoInfoPanel,remarkSet]
	});
                ...
                Ext.lib.Ajax.request('POST', 'comm/workticketApproveCheckUser.action',
                    {success : function(action) {
                        ...
                        if (result) {
                           //----------审批------------------------
            form.getForm().submit({
			url : 'bqworkticket/safetyExeApprove.action',
			waitMsg : '正在保存数据...',
			method : 'post',
			params : {
				workticketNo : strBusiNo,
		       ...
		       Ext.lib.Ajax.request('POST',
				'bqworkticket/getWorkticketBaseInfoByNo.action', {
					success : function(result, request) {
						...
						workticketMemo.setValue(record.data.model.workticketMemo);
