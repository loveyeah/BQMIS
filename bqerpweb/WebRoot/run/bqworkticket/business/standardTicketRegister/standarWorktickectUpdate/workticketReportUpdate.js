...
Ext.onReady(function() {
	...
	var headTbar = new Ext.Toolbar({
		region : 'north',
		items : ["工作票种类:", worktiketTypeCbo, "所属机组或系统:", equBlock,
				// "状态:",
				// workticketStatusCbo,
				'-', '填写人：', txtEnterBy, '-', {
					id : 'btnReflesh',
	...
	function modifyBtn() {
		...
		if (selected.length == 0) {
			...
			var menber = selected[0];
			workticketNo = menber.get('model.workticketNo');
			if (workticketNo.substring(0, 5) == "125MW") {
				...
				Ext.Ajax.request({
					url : 'workticket/createNewStandarWorktickect.action',
					method : 'post',
					params : {
						worktickectNo : workticketNo
					},
					success : function(result, request) {
						register.workFlowNo = menber.get('model.workFlowNo');
						register.firelevelId = menber.get('model.firelevelId');
						// 编辑工作票信息
						register.edit(result.responseText);
						searchBtn();
					},
					failure : function(result, request) {
					}
