...
Ext.onReady(function(){
	var fireLevel=window.dialogArguments.fireLevel;  //add by fyyang 090730
	// 系统当天日期
	var sd = new Date();
	var ed = new Date();
	// 系统当天前30天的日期
        ...
        handler : function() {
        	...
        	if(checkDate()) {
	            ...
	            runGridStore.baseParams = {
			        ...
			        fireLevel:fireLevel ,  //add by fyyang 090730
...
//        		{xtype : "tbseparator"}, '类型：', workticketTypeCbo, '-','所属机组或系统：', equBlock,
            	...
            	//{xtype : "tbseparator"}, '工作票编号：', txtNoContent,
            	{xtype : "tbseparator"}, '工作内容：', txtContent,
	...
	var headTbar = new Ext.Toolbar({
		...
		items : ['时间范围：', planStartDate, '~', planEndDate, '-', '类型：',
				workticketTypeCbo, '-','种类:',standardComboBox,'-', '所属机组或系统：', equBlock]
				...
				var obj = window.dialogArguments;
					Ext.Msg.confirm(Constants.SYS_REMIND_MSG, '是否确定选择工作票编号为' + object.refWorkticketNo + '的票',
	                    ...
	                    if (buttonobj == "yes") {
	                    	...
	                    	if(obj.workticketNo!=null&&obj.workticketNo!="")
	                    	{
							...
							Ext.lib.Ajax.request('POST',
								...
								success : function(action) {
									...
									var args=new Object();
			                    	args.hisWorkticketNo=member.get('model.workticketNo');
			                    	args.workticketContent=member.get('model.workticketContent');
							...
							}, "workticketNoNew=" + obj.workticketNo
