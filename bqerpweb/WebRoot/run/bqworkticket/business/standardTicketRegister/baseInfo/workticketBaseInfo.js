...
//		style:'cursor:hand;',
	...
	//	emptyText : '双击填写工作内容',
		...
		handler : function() {
			...
			var args = new Object();
			args.fireLevel=Ext.get("workticketBaseInfo.firelevelId").dom.value; //add by fyyang 090730
			args.workticketNo = txtWorkticketNo.getValue();
			args.workticketTypeCode = Ext.get("workticketBaseInfo.workticketTypeCode").dom.value;
			var object = window
					.showModalDialog(
	...
	var baseInfoField = new Ext.form.FieldSet({
		...
		items : [
				// 工作票编号
				{
					...
					value : '',
					xtype : 'hidden'
				}, txtWorkticketNo, cbxWorkticketType, 
				//add
				{
                                        id : 'hideMainEquCode',
                   ...
                   cbxMainEqu,
				cbxWorkticketSource,
				cbxRepairSpecail, cbxReceiveSpecail, cbxWorkCondition,
				cbxChargeBySystem, locationName, dangerCondition, dangerType,
				// //需要退出热工保护或自动装置名称  add by ywliu 090508
				autoDeviceName,
//				{
//					layout : "column",
...
//							}
	...
	//	height : 100,
		items : [new Ext.Toolbar.Fill(), {
			text : '由终结票生成',
			handler : function() {
				  var object = window.showModalDialog('endTicketSelect.jsp',
                null, 'dialogWidth=750px;dialogHeight=400px;center=yes;help=no;resizable=no;status=no;');
	...
	function getPageData(workticketNo) {
		...
		Ext.lib.Ajax.request('POST',
				'workticket/getDetailTicketInfoByCode.action', {
					success : function(result, request) {
						var record = eval('(' + result.responseText + ')');
								txtWorkContent.setValue(record.data.model.workticketContent);
						// 工作票号
...
//						{
							Ext.getCmp("hideMainEquCode").setValue(record.data.model.mainEquCode);
...
//						}
	...
	function checkInput() {
		...
		if(Ext.get("workticketBaseInfo.workticketTypeCode").dom.value==4 && cbxFireLevel.getValue()==""){
			msg += "{动火票级别}";
		}
		if (msg != '') {
...
//					cbxRepairSpecail.disable();
	...
	function nextTab() {
		...
		// 设置工作票种类/工作票类型编码
		register.workticketTypeCode = Ext.get('workticketBaseInfo.workticketTypeCode').dom.value;
		// 迁移到下一个页面
