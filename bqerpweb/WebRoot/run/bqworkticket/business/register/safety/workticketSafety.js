...
Ext.onReady(function() {
	...
	function reportBtn() {
		var isReport;
		var workticketType=strWorkticketType;
		var flowCode="";
		...
		Ext.lib.Ajax.request('POST', 'workticket/getReportWorkticket.action', {
			success : function(result, request) {
				...
				if (isReport != Constants.WORKTICKET_STAUS_ID_NotReport
					...
					flowCode=getFlowCode(workticketType,register.firelevel);
				var args=new Object();
		...
		var record = popGrid.selModel.getSelected();
			...
			if(names.length > 1) 
			{
				for(var i=1;i<names.length;i++)
				{
					var currentRecord = popGrid.getSelectionModel().getSelected();
					var count = popDs.getCount(); 
					var currentIndex = currentRecord?(currentRecord.get("operationOrder")+i):count 
					addRecord(currentIndex,names[i],codes[i],'。');
				}
				resetLine(); 
     			popGrid.startEditing(currentIndex, 2);
...
//		}
	...
	function addHandler() {
            var currentRecord = popGrid.getSelectionModel().getSelected();
				var count = popDs.getCount(); 
				var currentIndex = currentRecord?currentRecord.get("operationOrder"):count 
...
//                     	grid.getTopToolbar().items.get('myUpdate').setDisabled(true);
//                     }
//                     else if(record.data.isStandard=="Y")
//                     {
//                     	grid.getTopToolbar().items.get('upload').setDisabled(true);
//                     	grid.getTopToolbar().items.get('myUpdate').setDisabled(true);
//                     }
