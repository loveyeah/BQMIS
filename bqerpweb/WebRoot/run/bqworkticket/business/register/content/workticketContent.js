...
Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([sm, new Ext.grid.RowNumberer({
				...
				editor : new Ext.form.ComboBox({
					tpl : '<tpl for="."><div class="x-combo-list-item"  qtip="{contentKeyName}">{contentKeyName}</div></tpl>',
					typeAhead : true,
					editable : true,
					triggerAction : 'all',
					name : 'backKeyDesc',
	...
	function selectWorkticketContent() {
		...
		// var recordslen = store.data.items.length;
		// Ext.Ajax.request({
		...
		// success : function(result, request) {
		// // 查询成功后载入数据
		// var describeData = result.responseText;
		// describeData = Ext.util.JSON.decode(describeData);
		// contentDescribe.setValue(describeData);
		// }
		// })
	}

		// ----add by fyyang 090217------------
		//判断是否标准票，标准票不能修改 //-----------------判断是否由标准票生成，由标准票生成的不能修改

		// if(register.isCreateByStandard=="Y"||register.isStandard=="Y")
	 if(register.isStandard=="Y")
		 {
		 grid.getTopToolbar().items.get('btnGridAdd').setDisabled(true);
		 grid.getTopToolbar().items.get('btnGridSave').setDisabled(true);
		 grid.getTopToolbar().items.get('btnGridDelete').setDisabled(true);
		 grid.getTopToolbar().items.get('btnGridCancel').setDisabled(true);
		 grid.getTopToolbar().items.get('btnGridViewUpdate').setDisabled(true);
		 grid.getTopToolbar().items.get('add').setDisabled(true);
		 
		 }
		 else if(register.isCreateByStandard=="Y")
		 {
		 	grid.getTopToolbar().items.get('add').setDisabled(true);
		 }
		 else
		 {
		 grid.getTopToolbar().items.get('btnGridAdd').setDisabled(false);
		 grid.getTopToolbar().items.get('btnGridSave').setDisabled(false);
		 grid.getTopToolbar().items.get('btnGridDelete').setDisabled(false);
		 grid.getTopToolbar().items.get('btnGridCancel').setDisabled(false);
		 grid.getTopToolbar().items.get('btnGridViewUpdate').setDisabled(false);
		 grid.getTopToolbar().items.get('add').setDisabled(false);
...
//		 if(record.data.isCreatebyStand=="Y"||record.data.isStandard=="Y")
//		 {
//		 grid.getTopToolbar().items.get('btnGridAdd').setDisabled(true);
//		 grid.getTopToolbar().items.get('btnGridSave').setDisabled(true);
//		 grid.getTopToolbar().items.get('btnGridDelete').setDisabled(true);
