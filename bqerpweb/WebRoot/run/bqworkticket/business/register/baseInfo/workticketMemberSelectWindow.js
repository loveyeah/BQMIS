...
Ext.onReady(function() {
   ...
   var workticketNo =args.workticketNo;
   var  chargeDeptCode=args.chargeDept;
	var chargeDeptId="";
	var chargeDeptName="";
     //add by fyyang 090306
    var txtWorker=new Ext.form.TextField({
    id:'txtWorker',
    name:'txtWorker',
   hideLabel : true,
   anchor : "95%"
      ...
      function query()
      {
      	   staff_ds.load({
                                    params : {
										...
										deptId : comboDepChoose.value,
										queryKey:txtWorker.getValue()
                                    }
                                });
      }
    // 部门选择树下拉框
	...
	conn.open("POST", 'workticket/getDeptIdByCode.action?deptCode='
			+ chargeDeptCode, false);
	conn.send(null);
	...
	if (conn.status == "200") {
		...
		if (result.deptId) {
			chargeDeptId = result.deptId; 
			chargeDeptName=result.deptName;
...
//            })
    ...
    staff_ds.setDefaultSort('workerCode','asc');
    //初始载入数据
    staff_ds.load({
                                    params : {
                                        start : 0,
    ...
    staff_ds.on('beforeload', function() {
...
//			});
	...
	if (workticketNo== "" && storeData != null) {
		for (var i = 0; i < storeData.length; i++) {
			actor_ds.add(new Actor(storeData[i]));
		}
	
	}else
	{ 
		if(workticketNo != null && workticketNo != "")
		{ 
			actor_ds.load({
				params : {
					workticketNo : workticketNo,
    ...
    var grantBar = new Ext.Button({
        ...
        handler : function() {
            ...
            for (i = 0; i < selectedRows.length; i++) {
                ...
                names += selectedRows[i].data.workerName + ",";
            }
            if (ids.length > 0) {
                ...
                for (i = 0; i < selectedRows.length; i++) {
                    ...
                    for (j = 0; j < actor_ds.data.items.length; j++) {
                        // 已经添加过
                        if (wr.data.workerCode == actor_ds.getAt(j)
                                .get("actor.actorCode")) {
                            added = true;
                            break;
                        }
                    ...
                    if (added == false) {
                        addnames += wr.data.workerName + ",";
                        addids += wr.data.workerCode + ",";
                    } else {
                        // 已添加的放入addednames和addedids
                        addednames += wr.data.workerName + ",";
                        addedids += wr.data.workerCode + ",";
                    }
                    added = false;
                }
...
//                        Ext.Msg.alert("提示", "以下员工已经添加过！" + addednames);
                ...
                else
                {
                	//票号不存在时
                	  ...
                	  var  actorNames=names.split(',');
                	 
                	  if(names!=""&&actorNames.length>0)
                	  {
                	  var  actorCodes=ids.split(',');
                	  for( var i=0;i<actorNames.length;i++)
                	  {
                	  	var rec =new Actor({
                	  	id:null,
                	  	'actor.actorCode':actorCodes[i],
                	  	'actor.actorName':actorNames[i],
                	  	deptName:comboDepChoose.getValue(),
                	  	actortypename:'正式员工'
                	  })
                	    actor_ds.add(rec);
...
//                                                    o.msg);
            	...
            	else
            	{
            		//票号不存在时
            	 for (i = 0; i < selectedRows.length; i++) {
                     actor_ds.remove(selectedRows[i]);
                 }
...
//                        Ext.Msg.alert(Constants.NOTICE, o.msg);
    	...
    	else
    	{
    		var rec =new Actor({
                	  	id:null,
                	  	'actor.actorCode':null,
                	  	'actor.actorName':txtFloater.getValue(),
                	  	deptName:null,
                	  	actortypename:'临时员工'
                	  })
                	  actor_ds.add(rec);
    ...
    function onSubmit() {
    	...
    	else
    	{
    		   var actorRec = [];
			var actorNames = "";
			for (var i = 0; i < actor_ds.getCount(); i++) {
				actorRec.push(actor_ds.getAt(i).data);
				if (actorNames == "") {
					actorNames = actor_ds.getAt(i).get("actor.actorName");
				} else {
					actorNames += ","
							+ actor_ds.getAt(i).get("actor.actorName");
			...
			var selectedMembers = new Object();
			// 成员名字
			selectedMembers.names = actorNames;
			// 成员数量
			selectedMembers.quantity = actor_ds.getCount() + 1;
			selectedMembers.storeData = actorRec;
			window.returnValue = selectedMembers;
			window.close();
