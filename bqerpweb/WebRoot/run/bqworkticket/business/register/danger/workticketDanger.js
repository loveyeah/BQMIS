...
Ext.onReady(function() {
	...
	function savedanger() {
		...
		dangerpanel.getForm().submit({
			...
			success : function(form, action) {
				...
				// dangerId : o.id,
				// dangerName : Ext.get("dangerName").dom.value,
				// dangerMeasure : '',
				// orderBy : Ext.get("orderBy").dom.value
				// });
				// Ext.get("dangerContentId").dom.value = o.id;
				// var num = dstore.getTotalCount()
				// dstore.insert(num, myrec)
				// } else {
				// dstore.getAt(rownum).set('dangerName',
				// Ext.get("dangerName").dom.value)
				// } 
				store.reload();
	...
	if (register.isStandard == "Y") {
		...
		grid.getTopToolbar().items.get('btnGridUpdate').setDisabled(true);

		grid.getTopToolbar().items.get('btnGridDelete').setDisabled(true);
		createType = "Y";
	} else {
		grid.getTopToolbar().items.get('btnGridAdd').setDisabled(false);
		grid.getTopToolbar().items.get('btnGridUpdate').setDisabled(false);
		...
		//
		// if (record.data.isCreatebyStand == "Y"
		// || record.data.isStandard == "Y") {
