	...
	fieldLabel : '负责人',
		...
		onTriggerClick : function() {
			...
			if (typeof(emp) != "undefined") {
				...
				Ext.getCmp('chargeDep').setValue(emp.deptName);
				Ext.get("chargeDepH").dom.value=emp.deptCode;
...
//		name : 'spjs.chargeDep',
    ...
    var fillBy ={
		...
		onTriggerClick : function() {
			...
			if (typeof(emp) != "undefined") {
				Ext.getCmp('fillBy').setValue(emp.workerCode);
				Ext.form.ComboBox.superclass.setValue.call(Ext
						.getCmp('fillBy'), emp.workerName);
				Ext.getCmp('fillDep').setValue(emp.deptName);
				Ext.get('fillDepH').dom.value=emp.deptCode
			}
		}
	};
	...
	var addPanel = new Ext.FormPanel({
				...
				items : [
					...
					,{
						...
						labelWidth : 80,
						hidden : true,
						items : [finishState,finishAppraisal,appraisalBy,appraisalDate]
					}
				     ]
			});
...
//			
	...
	function deleteRecord() {
		...
		if (rec.length > 0) {
			for (var i = 0; i < rec.length; i++) {
				var member = rec[i];
				if (member.get('spjs.securityPlanId')) {
					ids.push(member.get('spjs.securityPlanId'));
				} else {
				}
			}
					...
					function(buttonobj) {
						if (buttonobj == 'yes') {
							...
							Ext.Ajax.request({
								...
								url : 'security/deleteSpJSecurityPlanInfo.action',
								params : {
									ids : ids.toString()
								},
	...
	function checkAddform() {
		...
		var charge = Ext.get('chargeBy').dom.value
		if(charge =="" || charge ==null)
		{
			strMsg += String.format(Constants.COM_E_003, '"负责人" ') + "<br/>";
		}
		var fill = Ext.get('fillBy').dom.value
		if(fill == "" || fill == null)
		{
			strMsg += String.format(Constants.COM_E_003, '"填报人" ') + "<br/>";
		}
		if (strMsg == "") {
			return true;
		} else {
		...
		Ext.Msg.confirm(Constants.SYS_REMIND_MSG, Constants.COM_C_001,
				function(buttonobj) {
					if (buttonobj == 'yes') {
							...
							if (strMethod == "add") {
								if (checkAddform()) {
									myurl = 'security/addSpJSecurityPlanInfo.action';
									{
										addPanel.getForm().submit({
											...
											success : function(form, action) {
												...
												Ext.Msg.alert("提示信息",result.msg);
													txtFuzzy1.setValue("");
		                                              queryStore.baseParams = {
														queryString : txtFuzzy1.getValue()
																	};
	 												  queryStore.load({
																params : {
																start : 0,
