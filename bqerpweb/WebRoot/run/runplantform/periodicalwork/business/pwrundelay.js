...
Ext.onReady(function() {
	...
	// 执行累呗
	var search_data2 = [["已申请延期", "Y"], ["未申请延期", "N"], ["全部", ""]];
	...
	var btnApply = new Ext.Button({
				...
				handler : function() {
					...
					if (CKSelectone()) {
						alert(rec.get("timeworkjInfo.status"));
						if (rec.get("timeworkjInfo.ifdelay") == "Y"
								&& rec.get("timeworkjInfo.delayresult") == "1") {
							Ext.Msg.alert('提示信息', "本工作已经申请延期");
							return false;
						} else if (rec.get("timeworkjInfo.status") == "3"
								|| rec.get("timeworkjInfo.status") == "6"
								|| rec.get("timeworkjInfo.status") == "7") {
							Ext.Msg.alert('提示信息', "本工作已经执行或者结束");
							return false;
						} else if (rec.get("timeworkjInfo.delayresult") != null) {
							Ext.Msg.alert('提示信息', "本工作不可再延期");
							return false;
						} else {
							var rec = rungrid.getSelectionModel().getSelected();
							if (rec.get("timeworkjInfo.ifdelay") == "Y") {
								Ext.Msg.alert('提示信息', "已 经 延 迟");
								return false;
							} else {
								Ext.Ajax.request({
											waitMsg : '上报中,请稍后...',
											url : 'timework/askdelayTimeworkj.action',
											params : {
												method : "lock",
												id : rec
	...
	rungrids.on('beforeload', function() {
				Ext.apply(this.baseParams, {
							...
							queryDelayType : Ext.get("RungridIfChk").dom.value
						});
			});
	// 运行执行的Grid主体
