...
Ext.onReady(function() {
						...
						if (blockdForm.getForm().isValid()) {
							...
							if (re == "ok") {
								blockdForm.getForm().submit({
									waitMsg : '保存中,请稍后...',
									url : "timework/agreeTimeworkj.action",
									params : {
								...
								blockdForm.getForm().submit({
									waitMsg : '保存中,请稍后...',
									url : "timework/agreedelayTimeworkj.action",
									params : {
										id : rec.get("timeworkjInfo.id")
									},
									success : function(form, action) {
							...
							} else if (re == "canceldalay") {
								blockdForm.getForm().submit({
									waitMsg : '保存中,请稍后...',
									url : "timework/disagreedelayTimeworkj.action",
									params : {
										id : rec.get("timeworkjInfo.id")
									},
									success : function(form, action) {
								...
								Ext.MessageBox.alert('错误', '未 指 定 的 操 作!');
							}
						}
					}
				}, {
	...
	// 执行类别
	var search_data2 = [["直接执行", "N"], ["审批执行", "Y"], ["全部", ""]];
	...
	if (conn.responseText != "") {
		...
		search_data1 = eval('([[\"全部定期工作\",\"\"],[\"延期定期工作\",\"delay\"],[\"临时下发工作\",\"temp\"],[\"已结束定期工作\",\"ed\"]])');
	}
	...
	// 同意按钮
	var rungridagree = new Ext.Button({
				id : 'handup',
				text : '同意',
				iconCls : 'save',
				handler : function() {
					if (CKSelectone()) {
						...
						if (rec.get("timeworkjInfo.ifcheck") == "N") {
							Ext.Msg.alert('提示信息', "本工作无需审批");
							return false;
						} else if (rec.get("timeworkjInfo.status") == "1") {
							Ext.Msg.alert('提示信息', "只可审批已上报工作");
							return false;
						} else if (rec.get("timeworkjInfo.checkresult") != "0") {
							Ext.Msg.alert('提示信息', "本工作已审批");
							return false;
						} else {
							re = "ok";
							showAdddWindow();
	...
	// 取消按钮
	var rungridisagree = new Ext.Button({
				id : 'handup',
				text : '取消',
				iconCls : 'cancer',
				handler : function() {
	...
	// 同意延期按钮
	var rungridagreedelay = new Ext.Button({
				id : 'handup',
				text : '同意延期',
				iconCls : 'save',
				handler : function() {
					if (CKSelectone()) {
						var rec = rungrid.getSelectionModel().getSelected();
						if (rec.get("timeworkjInfo.ifdelay") != "Y") {
							Ext.Msg.alert('提示信息', "本工作无需延期审批");
							return false;
						} else if (rec.get("timeworkjInfo.status") == "3"
								|| rec.get("timeworkjInfo.status") == "6") {
							Ext.Msg.alert('提示信息', "本工作已经结束");
							return false;
						} else if (rec.get("timeworkjInfo.delayresult") != "1") {
							Ext.Msg.alert('提示信息', "本工作不处于待延期审批状态");
							return false;
						} else {
							re = "okdelay";
							showAdddWindow();
	...
	// 不同意按钮
	var rungridisagreedelay = new Ext.Button({
				id : 'handup',
				text : '不同意延期',
				iconCls : 'cancer',
				handler : function() {
	...
	var rungrid = new Ext.grid.GridPanel({
				...
				columns : [rungridsm, new Ext.grid.RowNumberer(), {
							...
							width : 50,
							sortable : false,
							dataIndex : 'timeworkjInfo.workExplain'
						}, {
							header : "备注",
							width : 50,
							sortable : false,
							dataIndex : 'timeworkjInfo.memo'
						}, {
							header : "审批结果",
							width : 30,
							sortable : false,
