...
Ext.onReady(function() {
	...
	var keywords = {
		...
		xtype : "textarea",
		fieldLabel : '搜索关键字',
		name : 'timeworktypeInfo.keywordsinsql',
		allowBlank : true,
		blankText : '说明...',
		anchor : '96%'
	};
	...
	function showAddWindow() {
								...
								if (blockForm.getForm().isValid())
									if (op == "insert") {
										blockForm.getForm().submit({
											waitMsg : '保存中,请稍后...',
											url : "timework/addTimeworktype.action",
											params : {
												method : "insert"
											},
										...
										blockForm.getForm().submit({
											waitMsg : '保存中,请稍后...',
											url : "timework/updateTimeworktype.action",
											params : {
												method : "edit",
												id : centergrid
														.getSelectionModel()
		...
		if (op == "insert") {
			// 新增时，赋初始值
			blockAddWindow.setTitle("新增工作类型");
			blockForm.getForm().reset();
		} else if (op == "edit") {
			blockAddWindow.setTitle("修改工作类型");
			blockAddWindow.show();
			...
			if (centergrid.getSelectionModel().getSelections().length > 0) {
				Ext.Msg.confirm('提示信息', '确认 锁定 选中记录?', function(button, text) {
							if (button == 'yes') {
								...
								Ext.Ajax.request({
											waitMsg : '锁定中,请稍后...',
											url : 'timework/lockTimeworktype.action',
											params : {
												method : "lock",
												ids : str
											},
			...
			if (centergrid.getSelectionModel().getSelections().length > 0) {
				Ext.Msg.confirm('提示信息', '确认 解锁 选中记录?', function(button, text) {
							if (button == 'yes') {
								...
								Ext.Ajax.request({
											waitMsg : '解锁中,请稍后...',
											url : 'timework/unlockTimeworktype.action',
											params : {
												method : "unlock",
												ids : str
											},
			...
			if (centergrid.getSelectionModel().getSelections().length > 0) {
				Ext.Msg.confirm('提示信息', '确认 删除 选中记录?', function(button, text) {
							if (button == 'yes') {
								...
								Ext.Ajax.request({
											waitMsg : '删除中,请稍后...',
											url : 'timework/deleteTimeworktype.action',
											params : {
												method : "delete",
												ids : str
											},
	...
	var centerds = new Ext.data.JsonStore({
				url : 'timework/getlistTimeworktype.action',
				fields : datalist
			});
	centerds.load();
