...
Ext.onReady(function() {
	Ext.QuickTips.init();
	  ...
	  var purRecord = window.dialogArguments;
	/**
	 * 金钱格式化
	 */
						...
						// 已入库数
						{
							...
							renderer : function(value, cellmeta, record, rowIndex, columnIndex,
								...
								if (rowIndex < store.getCount() - 1) {
									...
									var totalSum = 0;
									for (var i = 0; i < store.getCount() - 1; i++) {
										totalSum += store.getAt(i).get('purOrderDetailsRcvQty');
									}
									if (store.getAt(store.getCount() - 1).get('isNewRecord') == 'total') {
										store.getAt(store.getCount() - 1).set('purOrderDetailsRcvQty',
												totalSum);
									}
									return moneyFormat(value);
								} else {
						...
						// 待入库数 canQty
						{
							...
							renderer : function(value, cellmeta, record, rowIndex, columnIndex,
								...
								if (rowIndex < store.getCount() - 1) {
									...
									if (store.getAt(store.getCount() - 1).get('isNewRecord') == 'total') {
										store.getAt(store.getCount() - 1).set('waitQty',
												totalSum);
									}
									return moneyFormat(value);
								} else {
						...
						},// 金额
						{
							...
							renderer : function(value, cellmeta, record, rowIndex, columnIndex,
								...
								if (rowIndex < store.getCount() - 1) {
									...
									if (store.getAt(store.getCount() - 1).get('isNewRecord') == 'total') {
										store.getAt(store.getCount() - 1).set('accoutPrice',
												totalSum);
									}
									return moneyFormat(value);
								} else {
...
var detailPanel = new Ext.FormPanel({
				style : "padding-right:0px;padding-bottom:0px;margin-bottom:0px",
				labelAlign : 'right',
				items : [headForm]
			});
...
//			});			
	...
	function showRightPurchaseWarehouse() {
		...
		//	contactNo.setValue(record.data.contractNo);
			...
			// 日期
			if(record.data.orderDetailModifiedDate!=null)
			{
				var mydate=record.data.orderDetailModifiedDate;
			operateDate.setValue(mydate.substring(0,10));
			}
			// 操作员
			operator.setValue(record.data.buyerName);
			//发票号
