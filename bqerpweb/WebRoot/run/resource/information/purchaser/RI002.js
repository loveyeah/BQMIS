...
Ext.onReady(function(){
	...
	// 从弹出窗口选择人员
	txtBuyer.onTriggerClick = popupSelect;
	...
	var buyerGrid = new Ext.grid.EditorGridPanel({
 	...
 	// beforeedit 事件
    buyerGrid.on('beforeedit', buyerBeforeEdit);

    // 单击一行事件
    buyerGrid.on('rowclick', buyerGridClick);
    ...
    var selectedStore = new Ext.data.JsonStore({
        url : 'resource/getSelectedMaterialList.action',
        root : 'list',
        sortInfo :{field: "no", direction: "ASC"},
        fields : selectedData
  	});
	...
	function getNodeToGrid(node) {
		if(node.id != -1) {
			var record = new waitSelectData({
	            ...
	            // 编码
	            no : node.id,
	            // 种类 modify by drdu 090630
	            kind : Ext.get('materialTypeRadio').dom.value
	 ...
	 * 单击一行，更新已选项
	 */
	function buyerGridClick() {
		if (buyerGrid.selModel.hasSelection()) {
			// 移除已选项的所有数据
			selectedStore.removeAll();
			var records = buyerGrid.selModel.getSelections();
			// 当前选择行
			selectedRow = buyerStore.indexOf(records[0]);
			// 如果此行对应的已选项已经更新过
	...
	function addRecord() {
        ...
        selectedStore.removeAll();
        // 原数据个数
        var count = buyerStore.getCount();
        // 停止原来编辑
        buyerGrid.stopEditing();
        // 插入新数据
        buyerStore.insert(count,record);
	...
	function deleteRecord() {
		var smodel = buyerGrid.getSelectionModel();
				...
				if (buttonobj == "yes") {
					...
					if(member.flag == "Y") {
						Ext.lib.Ajax.request(Constants.POST,
							'resource/deletePurchaserByBuyer.action', {
	...
	function afterDelete(record) {
		...
		var index = buyerStore.indexOf(record);
		// 从Grid中删除对应的数据
		buyerStore.remove(record);
	...
	function saveRecords() {
		...
		// 如果画面数据没有修改
		if((strString.length == 0) && (buyerCount < 1)) {
			...
			buttonobj) {
				...
				if (buttonobj == "yes") {
					...
					if(checkGrid() && checkSelected()) {
						Ext.lib.Ajax.request(Constants.POST,
								'resource/saveBuyerRecords.action', {
									success : function(action) {
	...
	function checkGrid() {
		var count = buyerStore.getCount();
		if(checkBuyerEmpty() && checkBuyerRepeat()) {
			return true;
		} else {
			return false;
		}
	...
	function checkSelected() {
		if(emptyCount.length > 0) {
			var msg = "";
			for(var i = 0; i < emptyCount.length; i++) {
				var record = buyerStore.getAt(emptyCount[i]);
				if(msg != "") {
					msg += '<br/>' + String.format(Constants.COM_E_017, '人员编码',record.get("buyer"),'已选项列表');
				} else {
					msg += String.format(Constants.COM_E_017, '人员编码',record.get("buyer"),'已选项列表');
				}
			}
			if(msg != "") {
	 ...
	 * 检查Grid内数据是否为空
	 */
	function checkBuyerEmpty() {
		var count = buyerStore.getCount();
		for(var i = 0; i < count; i++) {
			var buyer = buyerStore.getAt(i).get("buyer");
			if(buyer == "" || buyer == null) {
	 ...
	 * 检查Grid内数据是否重复
	 */
	function checkBuyerRepeat() {
		var count = buyerStore.getCount();
		for(var i = 0; i < count; i++) {
			var record = buyerStore.getAt(i);
			for(var j = i + 1; j < count; j++) {
				var recordCompare = buyerStore.getAt(j);
				if(record.get("buyer") == recordCompare.get("buyer")) {
	...
	function toRight() {
		...
		var buyerSelected = buyerGrid.selModel.getSelections();
		// 如果没有选中行
		if (buyerSelected.length < 1) {
			Ext.Msg.alert(Constants.SYS_REMIND_MSG, Constants.RI002_E_004);
		} else {
	...
	function changeHandler() {
		...
		if (buyerGrid.selModel.hasSelection()) {
			...
			if(count == 0) {
				...
				if(buyerStore.getAt(selectedRow).get("flag") == "Y") {
					var obj = new Object();
					obj.names = new Array(" ");
					obj.nos = new Array(" ");
					obj.kinds = new Array(" ");
					obj.buyer = buyerStore.getAt(selectedRow).get("buyer");
					obj.buyerName = buyerStore.getAt(selectedRow).get("buyerName");
					obj.flag = buyerStore.getAt(selectedRow).get("flag");
	...
	function popupSelect() {
		...
		if (object) {
			var record = buyerStore.getAt(selectedRow);
			if (typeof(object.workerCode) != "undefined") {
				Ext.Ajax.request({
					url : 'resource/getPersonInfoBuyer.action',
					method : Constants.POST,
					params : {
						// 人员编码
						buyer : object.workerCode
					},
					// 成功
					success : function(result, request) {
						result = eval("(" + result.responseText + ")");
						record.set("buyer", object.workerCode);
						if(typeof(gridObjects[selectedRow]) != "undefined") {
							gridObjects[selectedRow].buyer = object.workerCode;
						}
						record.set("buyerName", object.workerName);
						if(typeof(gridObjects[selectedRow]) != "undefined") {
							gridObjects[selectedRow].buyerName = object.workerName;
						}
						if(result != null) {
