...
Ext.onReady(function() {
	...
	// 部门负责人 modify by dswang 20090617
...
//									+ id + "&type=MCON");
					var viewUrl = "managecontract/showConFile.action?conid="+ id + "&type=MCON"; 
				    var params = {
				...
				items : [{
					...
					items : [{
								...
								items : [{
											...
											columnWidth : 0.3,
											layout : 'form',
											items : [modiyActAmount, hidunitBox]
										}]
							}
							, {
								layout : 'column',
	...
	function initBase() {
		...
		btnSet(false);
		// var url =
		// "manage/contract/business/modifyMeetSign/register/tabPayPlan/payplan.jsp";
		// parent.document.all.iframe3.src = url + "?id=";
	}
	// addBy ywliu 09/04/24
	function newBase() {
		check = "add";
	...
	function saveBase() {
		...
		if (check == "add") {
			...
			// + "&fileName=" + Ext.get("oriFileName").dom.value;
		} else {
		...
		myform.getForm().submit({
					...
					success : function(form, action) {
						...
						// url2 =
						// "manage/contract/business/modifyMeetSign/register/tabPayPlan/payplan.jsp";
						// parent.document.all.iframe3.src = url2 + "?id=" + id
							...
							Ext.lib.Ajax.request('POST',
									'managecontract/deleteModifyBase.action', {
										success : function(action) {
											...
											initBase();
											btnView.setDisabled(true);
											url = "manage/contract/business/modifyMeetSign/register/tabList/list.jsp";
	...
	function conModifyReport() {
		...
		if (execFlag == 2) {
			Ext.Msg.alert('提示', '本合同在变更中!');
			return false;
		}
		// Ext.Ajax.request({
		// url : 'managecontract/conModApproveReport.action',
		// params : {
		// conModId : id
		// },
		// method : 'post',
		...
		args.conModId = id;
		args.sum = modiyActAmount.getValue();
...
//				tbar : Materialtbar,
	...
	function CheckRptPreview() {
		if (conId == null || conId == "") {
			...
			var url = "/powerrpt/report/webfile/CGContractModifyMeetSign.jsp?modifyId="
					+ id;
			window.open(url);
		}
	...
	function btnSet(status) {
		...
		//隐掉物资明细的增加删除
//		MaterialGrid.getTopToolbar().items.get('btnCredentSel')
//				.setVisible(status);
//		MaterialGrid.getTopToolbar().items.get('btnCredentDelete')
//				.setVisible(status);
	}
