...
Ext.onReady(function() {
	...
	var modelItemId = args.modelItemId;
	...
	var dataProxy = new Ext.data.HttpProxy({
				url : 'managebudget/getModelFormulaList.action'
			});
	...
	function saveModifies() {
		...
		if (modifyRec.length > 0 || isDeletes) {
			...
			for (var i = 0; i < grid.getStore().getCount(); i++) {
				...
				rec.modelItemId = obj.get("modelItemId");
				updateData.push(rec);
			}
			Ext.Ajax.request({
						url : 'managebudget/saveOrupdateModelFormula.action',
						method : 'post',
						params : {
							modelItemId : modelItemId,
	...
	var con_ds = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'managebudget/getAllmodelItemList.action'
						}),
	...
	Grid.on("rowclick", function() {
				...
				if (recR.get('modelItemId') == modelItemId) {
					Ext.Msg.alert("提示", "指标公式不能包含该指标本身！");
					return;
				}
				var recL = grid.getSelectionModel().getSelected();
				if (recL) {
					recL.set("rowItemCode", recR.get('modelItemCode'));
					recL.set("formulaContent", recR.get('modelItemName'));
				} else {
	...
	var deptTree = new Ext.tree.TreePanel({
				...
				loader : new Ext.tree.TreeLoader({
							dataUrl : 'managebudget/findModelTreeList.action'
						}),
				root : rootNodeObj,
				autoWidth : true,
				layout : 'fit',
	...
	deptTree.on('beforeload', function(node) {
				deptTree.loader.dataUrl = 'managebudget/findModelTreeList.action?pid='
						+ node.id;
			});
