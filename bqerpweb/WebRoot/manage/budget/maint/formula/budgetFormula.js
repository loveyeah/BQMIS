...
Ext.onReady(function() {
	...
	var dataProxy = new Ext.data.HttpProxy({
		url : 'managebudget/getCbmCItemFormulaList.action'
	});
	...
	function saveModifies() {
		...
		if (modifyRec.length > 0 || isDeletes) {
			...
			var itemExplain = Ext.get("winspace").dom.value;
			...
			Ext.Ajax.request({
				url : 'managebudget/saveCbmCItemFormula.action',
				method : 'post',
				params : {
					itemId : itemId,
					isUpdate : Ext.util.JSON.encode(updateData),
					itemExplain : Ext.util.JSON.encode(itemExplain)
				},
	...
	function chooseValue() {
		var records = operateGrid.getSelectionModel().getSelections();
		if (records.length == 1) {
			win.hide();
		} else {
	...
	var operateGrid = new Ext.grid.GridPanel({
		store : operateStore,
		layout : 'fit',
		listeners : {
	...
	win.on('hide', function() {
		...
		var selected = operateGrid.getSelectionModel().getSelections();
		var opeRec = operateGrid.getSelectionModel().getSelected();
		if(selected.length==1){
			rec.set("rowItemCode", opeRec.data.key);
			rec.set("formulaContent", opeRec.data.value);
		}
	...
	var oprBut = new Ext.form.TextField({
		readOnly : true,
		emptyText:'请选择...',
		listeners:{
			'focus':function(){
	...
	function choseEdit(rightGrid, rowIndex, columnIndex, e) {
		...
		var type = record.data.fornulaType;
		if (type == 3) {
			win.show();
		}
...
//								new Ext.grid.GridEditor(oprBut));
//								oprBut.blur();
						
						grid.getColumnModel().setEditor(2,
								new Ext.grid.GridEditor(oprBut));
						grid.on('cellclick',choseEdit );
	...
	var con_ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'managebudget/getAllStatItemList.action'
		}),
	...
	deptTree.on('beforeload', function(node) {
		deptTree.loader.dataUrl = 'managebudget/findBudgetTree.action?pid='
	...
	deptTree.on('dblclick', function(node, e) {
		...
		Ext.Ajax.request({
			...
			success : function(result, request) {
				...
				if (isItem) {
					if (o.zbbmtxCode == itemCode) {
						Ext.Msg.alert("提示", "指标公式不能包含该指标本身！");
						return;
					}
					var recL = grid.getSelectionModel().getSelected();
					if (recL && recL.get("fornulaType") == 1) {
						recL.set("rowItemCode", o.zbbmtxCode);
