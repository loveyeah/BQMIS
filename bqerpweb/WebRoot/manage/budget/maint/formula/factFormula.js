...
Ext.onReady(function() {
	...
	var dataProxy = new Ext.data.HttpProxy({
		url : 'managebudget/getCbmCFactFormulaList.action'
	});
	...
	function saveModifies() {
		...
		if (modifyRec.length > 0 || isDeletes) {
			...
			var factExplain=Ext.get("winspace").dom.value;
			...
			Ext.Ajax.request({
				url : 'managebudget/saveCbmCFactFormula.action',
				method : 'post',
				params : {
					itemId : itemId,
					isUpdate : Ext.util.JSON.encode(updateData),
					factExplain:factExplain
				},
	...
	var deptTree = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			dataUrl : 'managebudget/findBudgetTree.action'
		}),
		root : rootNodeObj,
		autoWidth : true,
		layout : 'fit',
