...
Ext.onReady(function() {
  ...
  /*var electricMonitor = {
		...
		mode : 'remote',
		hiddenName : 'power.electricMonitor',
		//allowBlank : false,
		editable : false,
		anchor : "80%",
		triggerAction : 'all',
		...
		var args = {
			...
			var emp = window.showModalDialog(url, args,'dialogWidth:550px;dialogHeight:300px;directories:yes;help:no;status:no;resizable:no;scrollbars:yes;');

			if (typeof(emp) != "undefined") 
			{
					Ext.getCmp('electricMonitor').setValue(emp.workerCode);
				   Ext.form.ComboBox.superclass.setValue.call(Ext
						.getCmp('electricMonitor'),emp.workerName);
		   }
		}
	    };*/
		...
		mode : 'remote',
		hiddenName : 'power.validateMonitor',
		//allowBlank : false,
		editable : false,
		anchor : "80%",
		triggerAction : 'all',
		...
		var args = {
			...
			if (typeof(emp) != "undefined") 
			{
					Ext.getCmp('validateMonitor').setValue(emp.workerCode);
				   Ext.form.ComboBox.superclass.setValue.call(Ext
						.getCmp('validateMonitor'),emp.workerName);
		   }
		}
	    };*/
        ...
        items : [{
					...
					layout : "form",
					border : false
					//items : [electricMonitor]
				},
				{
					columnWidth : 0.5,
					layout : "form",
					border : false
					//items : [validateMonitor]
				}]
    });*/
    
//      var finishDateLabel = new Ext.form.Label({
//        text : " 完成时间：",
//        style : 'font-size:12px'
//    });
    ...
    var form = new Ext.FormPanel({
        ...
        buttons : [{
                ...
                if(actionId ==78){    	
                 ...
                 /*if(eventIdentify!="TH")
                 {
            	if(Ext.get("electricMonitor").dom.value=="")
            	{
            		alertmsg="电气班长";
            	 	
            	}
            	if(Ext.get("validateMonitor").dom.value=="")
            	{
            		if(alertmsg!="")
            		{
            			alertmsg=alertmsg+",确认班长";
            		}
            		else
            		{
            		 alertmsg="确认班长";
        ...
        items : [titleField,tableField,showAppointNextRoles,approveSet, remarkSet]
    });
                ...
                Ext.lib.Ajax.request('POST', 'comm/workticketApproveCheckUser.action',
                    {success : function(action) {
                        ...
                        if (result) {
                           //----------审批------------------------
            form.getForm().submit({
			url : 'managebudget/approveSign.action',
			waitMsg : '正在保存数据...',
			method : 'post',
			params : {
				assignId : assignId,
...
//		Ext.Ajax.request({
//			url : "managebudget/getCurrentTitle.action",
//			method : 'post',
//			params : {
//				entryId : entryId
//			},
//			success : function(result, request) {
//				var mytitle = eval('(' + result.responseText + ')'); 
//			},
//			failure : function() {
//				
//			}
