...
Ext.onReady(function() {
	...
	function budgetBasisSelect() {
		...
		var url = "budgetBasisSelect.jsp?budgetItemId="+budgetItemId+ "&makeStatus="+ record.get("makeStatus")+ "&unit="+unit+"" ;
		if (budgetItemId == 0) {
			Ext.Msg.alert("提示","请先保存数据！");
				return;
			}else{
			...
			if (typeof(mate) != "undefined") {
				record.set('adviceBudget', mate.amountSum);
