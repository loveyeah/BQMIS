...
Ext.onReady(function() {
	...
	var westgrids = new Ext.data.JsonStore({
		url : 'managebudget/findModifyApproveList.action',
		root : 'list',
	...
	function modifySign() {
		...
		var workFlowNo = record.get('changeWorkFlowNo');
		Ext.Ajax.request({
			...
			success : function(result, request) {
				...
				if (obj[0].url==null||obj[0].url=="") {
					url="budgetModifySign.jsp";
				} 
				else
				{
					 url = "../../../../../" + obj[0].url; 
				} 
					args.busiNo = record.get('changeId');
	...
	function showModifyInfo() {
		...
		if (selrows.length > 0) {
			...
			if (changeId == null || changeId == "") {
				...
				url = "../budgetModify/budgetModifySelect/budgetModifySelect.jsp?budgetItemId="
