...
//				Ext.Msg.hide();
	...
	/*ds.on('load', function() {
		for (var j = 0; j < ds.getCount(); j++) {
					var temp = ds.getAt(j);
					if(temp.get("gatherStatus")==null||temp.get("gatherStatus")=="")
					{
					 temp.set("gatherStatus","0");
					}
					if(temp.get("gatherStatus")=="0")
					{
						Grid.getTopToolbar().items.get('save').setDisabled(false);
						Grid.getTopToolbar().items.get('Upcommit').setDisabled(false);
					}
					else
					{
						Grid.getTopToolbar().items.get('save').setDisabled(true);
						Grid.getTopToolbar().items.get('Upcommit').setDisabled(true);
					}
					if(temp.get("adviceBudget")==null||temp.get("adviceBudget")=="")
					{
