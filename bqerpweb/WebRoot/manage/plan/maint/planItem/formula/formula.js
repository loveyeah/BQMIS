...
Ext.onReady(function() {
	...
	var itemCode = args?args.itemCode:'3431';
	...
	var dataProxy = new Ext.data.HttpProxy({
				url : 'manageplan/getBpCPlanFormulaList.action'
			});
	...
	function saveModifies() {
		...
		if (modifyRec.length > 0 || isDeletes) {
			...
			Ext.Ajax.request({
						url : 'manageplan/saveBpCPlanFormula.action',
						method : 'post',
						params : {
	...
	var con_ds = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'manageplan/getPlanItemList.action'
						}),
	...
	deptTree.on('beforeload', function(node) {
				deptTree.loader.dataUrl = 'manageplan/getPlanItemTreeNode.action?pid='
						+ node.id;
			});
