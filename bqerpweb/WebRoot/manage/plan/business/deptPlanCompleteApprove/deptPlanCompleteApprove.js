...
Ext.onReady(function() {
	...
	var entryId2= entryId.substring(0,6);//add by sychen 20100408
...
//	var isEqu = arg.isEqu;
	...
	var graphButton = new Ext.Button({
				...
				handler : function() {
					var url;
					if (entryId2 == null || entryId2 == "")//update by sychen 20100408
	...
	var approveHistoryButton = new Ext.Button({
		...
		handler : function() {
			var url = "/power/workflow/manager/approveInfo/approveInfo.jsp?entryId="
					+ entryId2;//update by sychen 20100408
			window
					.showModalDialog(
							url,
							null,
	...
	function getNextSteps() {
		...
		args.entryId = entryId2;//update by sychen 20100408
		var url = "";
...
//		}
		...
		Ext.Ajax.request({
					...
					method : 'post',
					params : {
						entryId : entryId2,//update by sychen 20100408
				...
				Ext.lib.Ajax.request('POST',
						'comm/workticketApproveCheckUser.action', {
							success : function(action) {
								...
								if (result) {
									form.getForm().submit({
										url : 'manageplan/deptPlanCompleteApprove.action',
										waitMsg : '正在保存数据...',
										method : 'post',
										params : {
