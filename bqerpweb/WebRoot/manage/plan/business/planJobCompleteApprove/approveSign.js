...
Ext.onReady(function() {
	...
	if (selectFlag == "many") {
		var entryId2 = entryId.substring(0, 6);
	}
	else {
	...
	var finishStatus = arg.finishStatus;
	
	var ifDiversify = arg.ifDiversify;
	var ifHeating = arg.ifHeating;
	var deptMainId = arg.deptMainId;
	var deptMainIds=arg.deptMainIds;// add by sychen 20100707
	var workerCode = "";
...
//							disabled:disFlag
	...
	var form = new Ext.form.FormPanel({
				...
				buttons : [{
							...
							if (eventIdentify == ('TH')
									&& selectFlag == "single") {
								deptMainIds = deptMainId;
							}
							alert(deptMainIds)
							// add by sychen 20100707 end
							Ext.Msg.confirm('提示', '本部门月度计划确认已完善？',
									function(id) {
										if (id == 'yes') {// 显示用户验证窗口
											if (eventIdentify == ('TY（ZZGCLD）')) {
												if (nrs == "") {
													Ext.Msg.alert("提示",
															"请选择主管厂领导具体人员!");
													return false;
				...
				Ext.lib.Ajax.request('POST',
						'comm/workticketApproveCheckUser.action', {
							success : function(action) {
								...
								if (result) {
									form.getForm().submit({
										url : 'manageplan/planJobCompleteApprove.action',
										waitMsg : '正在保存数据...',
										method : 'post',
										params : {
//											deptMainId : deptMainId,
											deptMainId : deptMainIds,// add by sychen 20100707
											nextRoles : nrs,
											actionId : actionId,
											workflowType : workflowType,
											entryId : entryId,
											approveText : remark.getValue(),
											ifDiversify:ifDiversify,
											ifHeating:ifHeating, 
