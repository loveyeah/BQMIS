...
Ext.onReady(function() {
	...
	function getUrl(workflowType) {
		switch (workflowType) {
			// case 'bqDeptJobplan' :
			case 'bqDeptWorkPlanApprove' :// update by sychen 20100326
				return "manageplan/workPlanCommit.action";

			case 'bqDeptJobplangather' :
				return "manageplan/workPlanGatherCommit.action";
			default :
				return "";
		}
	...
	function getTabTitle(n) {
		switch (n) {
			// case 'bqDeptJobplan' :
			case 'bqDeptWorkPlanApprove' : // update by sychen 20100325
				return "部门工作计划上报";
			case 'bqDeptJobplangather' :
				return "部门工作计划汇总上报";
			default :
	...
	var showWord = getTabTitle(workflowType);
	...
	var approveSet = new Ext.Panel({
				...
				border : false,
				layout : 'form',
				style : "padding-top:5;border-width:1px 0 0 0;",
				items : [approveLabel, radioSet]
			});
	// update by sychen 20100325
...
//						}]
//			})
	// if (workflowType == "bqDeptJobplan") {
//	 if (workflowType == "bqDeptWorkPlanApprove") {
//		equApprovePanel.show()
//	} else {
//		equApprovePanel.hide()
//	}
	/**
	 * 审批方式radio button生成
	 */

// 控制审批方式 显示上报 add by sychen 20100428 
	function radioAddHandler() {
		if (!radioSet.items) {
			radioSet.items = new Ext.util.MixedCollection();
		}
		var _radio = new Ext.form.Radio({
					boxLabel : "<font size='2.6px'>上报</font>",
					name : 'rb-approve',
					inputValue :24,
					listeners : {
						'check' : function() {
...
//							name : 'rb-approve',
////							inputValue : radio[0].actionId,//update by sychen 20100428
//							inputValue : 24,
//							url : radio[0].url,
...
//						radioAddHandler(radio.actions);
	...
	function selectRole() {
		form.getForm().submit({
					...
					params : {
						...
						deptId : deptId,// add by drdu 20100520
						nrs : nrs// add by drdu 20100520
						// ,//update by sychen 20100325
						// isEqu : isEqu
					},
	...
	var form = new Ext.form.FormPanel({
				...
				buttons : [{
							...
							handler : function() {
								// 显示用户验证窗口
								workerPanel.getForm().reset();// update by sychen 20100325
//								 if (workflowType == "bqDeptJobplan") {
				...
				Ext.lib.Ajax.request('POST',
						'comm/workticketApproveCheckUser.action', {
							success : function(action) {
								var result = eval(action.responseText);// update
								// by
								// sychen
								// 20100325
								// var isEqu = Ext.get("iy").dom.checked
								// ? "Y"
								// : "N";
								// 如果验证成功，进行签字操作
								if (result) {
									// //add by sychen 20100325
