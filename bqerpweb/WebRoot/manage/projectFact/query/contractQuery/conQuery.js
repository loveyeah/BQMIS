...
CheckRptPreview();
	...
	function myExport() {
		Ext.Ajax.request({
			url : 'managecontract/getAllConList.action?conTypeId=2',
			success : function(response) {
				var json = eval('(' + response.responseText.trim() + ')');
				var records = json.root;
				// alert(records.length);
				var html = ['<table border=1><tr><th>会签状态</th><th>合同编号</th><th>合同名称</th><th>合作伙伴</th><th>有无总金额</th><th>总金额</th><th>币别</th><th>有无变更</th><th>经办人</th><th>经办部门</th><th>会签开始时间</th><th>会签结束时间</th></tr>'];
			...
			items : [new Ext.menu.Item({
				...
				handler : function() {
					...
					if (workflowStatus == "0" || workflowStatus == "1"
							|| workflowStatus == "2") {
						strReportAdd = "/powerrpt/report/webfile/bqmis/contract.jsp?conId="
								+ conId;
					} else {
						strReportAdd = "E";
					}
