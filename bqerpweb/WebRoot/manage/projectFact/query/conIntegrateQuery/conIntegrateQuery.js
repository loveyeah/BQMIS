...
Ext.onReady(function() {
	...
	var btnQuery = new Ext.Button({
		...
		minWidth : 70,
		handler : function() {
			var _thisTitle = '<center><font color="blue" size="3">大唐陕西发电有限公司灞桥热电厂项目合同综合统计</font><br/><br/>'
		...
		handler : function() {
			...
			if (selrows.length > 0) {
				...
				if ((conModifyId != null) && (conModifyId != "")) {
					url = "../../../../manage/projectFact/business/conBaseInfo/conInfo.jsp?conId="
	...
	function myExport() {
		Ext.Ajax.request({
			url : 'managecontract/finConIntegrateList.action?conTypeId=' + 2,
			success : function(response) {
				var json = eval('(' + response.responseText.trim() + ')');
				var records = json.root;
				// alert(records.length);
				var html = ['<table border=1><tr><th>合同编号</th><th>合同名称</th><th>合作伙伴</th><th>有无总金额</th><th>总金额</th><th>币别</th><th>有无变更</th><th>合同状态</th><th>归档状态</th><th>经办人</th><th>经办部门</th><th>履行开始时间</th><th>履行结束时间</th></tr>'];
				for (var i = 0; i < records.length; i += 1) {
					var rc = records[i];
					html.push('<tr><td>' + rc.conttreesNo + '</td><td>'
							+ rc.contractName + '</td><td>' + rc.clientName
							+ '</td><td>' + rc.isSum + '</td><td>'
							+ rc.actAmount + '</td><td>' + rc.currencyType
	...
	var _title = '<center><font color="blue" size="3">大唐陕西发电有限公司灞桥热电厂项目合同综合统计</font><br/><br/>'
