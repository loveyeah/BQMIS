...
Ext.onReady(function() {
	...
	var form = new Ext.FormPanel({
		...
		buttons : [{
			...
			id : 'btnSign',
			handler : function() {
				var commValidate = new Power.validate();
				commValidate.win.show();
				commValidate.win.on("hide", function() {
					var result = commValidate.getValue();
					if (result != null && result.success) {
						// ----------审批------------------------
						form.getForm().submit({
							url : 'managexam/awardApproveSign.action',
							waitMsg : '正在保存数据...',
							method : 'post',
							params : {
								entryId : entryId,
