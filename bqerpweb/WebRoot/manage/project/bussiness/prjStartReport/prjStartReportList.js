...
Ext.onReady(function() {
		...
		handler : function() {
			if (repGrid.getSelectionModel().hasSelection()) {
				...
				if (rec.length > 1) {
					...
					var url = "/powerrpt/report/webfile/projectStartReport.jsp?reportId="
							+ record.data.reportId+"&backEntryBy="+record.data.backEntryBy;
					window.open(url);
				}
	...
	var repGrid = new Ext.grid.GridPanel({
		...
		tbar : ['开工时间：', start_date, '竣工时间：', end_date, '工程类别：', prj_type,
				queryButton, '-', saveButton, '-', updateButton, '-',
