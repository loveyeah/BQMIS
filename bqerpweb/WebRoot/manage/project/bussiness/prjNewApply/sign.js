...
Ext.onReady(function() {
	 ...
	 var dealContent = arg.con_no;// add by ypan 20100928 项目名称(工单内容)
	var dealType = arg.project;// add by ypan 20100928 项目(工单类型)
	var fillBy = arg.entryBy;// add by ypan 20100928 填写人(申请人)
	var depName = arg.depName// add by ypan 20100928 填写人所在的一级部门(检修部门)
	var applyStartDate=arg.applyStartDate//add by ypan 20100928 工程开始日期(申请工期)
	var applyEndDate=arg.applyEndDate;//add by ypan 20100928 工程结束日期(申请工期)
	...
	var lblApprove = new Ext.form.Label({
		...
		text : '相关领导审批',
		height : 10,
		style : "color:blue;line-height:100px;padding-left:50px;font-size:30px"
	});
...
//			}
	...
	var validateWin = new Ext.Window({
		...
		buttons : [{
			...
			id : 'btnSign',
			handler : function() {
//				alert("entryId:"+entryId+":actionId:"+actionId+":eventIdentify:"+eventIdentify);
				// 工号确认
				Ext.lib.Ajax.request('POST',
						'managecontract/contractApproveCheckUser.action', {
							success : function(action) {
								...
								if (result) {
									Ext.Ajax.request({
										url : 'manageproject/checkOperation.action',
										waitMsg : '正在保存数据...',
										method : 'post',
										params : {
											entryId : entryId,
											...
											applyId : id,
										  content:dealContent ,// add by ypan 20100928项目名称(工单内容)
	                                      type:dealType,// add by ypan 20100928 项目(工单类型)
	                                      by: fillBy ,// add by ypan 20100928 填写人(申请人)
	                                      name:depName ,// add by ypan 20100928 填写人所在的一级部门(检修部门)
                                          startDate:applyStartDate,// add by ypan 20100928 工程开始日期(申请工期)
	                                      endDate:applyEndDate// add by ypan 20100928 工程结束日期(申请工期)
										},
										success : function(result, request) {
