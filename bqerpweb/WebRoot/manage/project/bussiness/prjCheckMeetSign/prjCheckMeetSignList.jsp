<%@ page language="java" pageEncoding="utf-8"%>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
	<head>
		<title>工程项目验收会签单请</title>
		<jsp:include page="../../../../comm/jsp/commjsp.jsp"></jsp:include>
		<script type="text/javascript" src="comm/datepicker/WdatePicker.js"
			defer="defer"></script>
			
				<script type="text/javascript" src="comm/ext/ComboBoxTree.js"></script>
							<script type='text/javascript' src='comm/ext/FileUploadField.js'></script>
	<link rel="stylesheet" type="text/css" href="comm/ext/file-upload.css"/>
	</head>
	<body>
		<script type="text/javascript" src="../../../comm/ext/urlparams.js"></script>
		<script type="text/javascript"
			src="manage/project/maint/prjRegister/prjRegister.js" ></script>
		<script type="text/javascript"
			src="manage/project/bussiness/prjCheckMeetSign/prjCheckMeetSignForm.js"></script>
		<script type="text/javascript"
			src="manage/project/bussiness/prjCheckMeetSign/prjCheckMeetSignList.js"></script>

	</body>
</html>
