...
Ext.onReady(function() {
		...
		listeners : {
			focus : function() {
				WdatePicker({
							...
							dateFmt : 'yyyy-MM-dd',
							onpicked : function() {
								if (factEndDate.getValue() != "") {
									if (factStartDate.getValue() == ""
											|| factStartDate.getValue() > factEndDate
													.getValue()) {
										Ext.Msg.alert("提示", "必须小于时间结束时间");
										factStartDate.setValue("");
										return;
									}
									Ext.get("factTimeLimit").dom.value = DateDiff(
											factStartDate.getValue(),
											factEndDate.getValue())
											+ 1;
	...
	// 工程变更简述
	var prjChangeMemo = new Ext.form.TextArea({
				id : "prjChangeMemo",
				fieldLabel : '工程变更简述',
	...
	// 工程资料移交情况
	var prjDataMove = new Ext.form.TextArea({
				id : "prjDataMove",
				fieldLabel : '工程资料移交情况',
	...
	function AddProject() {
		var url;
		url = "manageproject/updateProjectCheck.action";
...
//		 alert(Ext.encode(parent.myRecord.data))
		...
		var prjStatus = parent.myRecord.get("prjjInfo.prjStatus")
		itemId.setValue((parent.myRecord.get("prjjInfo.itemId") == "zzfy")
				? "生产成本"
				: "劳务成本");
		if (prjStatus != 4 && prjStatus != 7) {
