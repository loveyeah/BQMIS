...
var fuzzy = new Ext.form.TextField({
	...
	fileType.on("beforequery", function() {
		if (checkFileId.getValue() != null && checkFileId.getValue() != "") {
			return false;
		} else {
			return true;
		}
	...
	var doccontent = new Ext.form.FieldSet({
		height : '100%',
		layout : 'form',
		items : [checkFileId, fileNo, fjdocFile, fileName, fileType,
				lastModifiedDate, lastModifiedName, isModel]
	});
	...
	var annextbar = new Ext.Toolbar({
		...
		items : ['名称：', fuzzy, {
			...
			handler : function() {
				...
				docwin.setTitle("增加竣工报告书");
				docform.getForm().reset();
				docwin.show();
				btnView.setVisible(false);
				// Ext.get("fjdocFile").dom.select();
				// document.selection.clear();
				Ext.get('isModel').dom.value = "N";
