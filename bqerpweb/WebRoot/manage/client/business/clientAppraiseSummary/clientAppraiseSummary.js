...
Ext.onReady(function() {
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			text : '取消',
			id : 'cancer',// modify by ywliu 添加ID
			iconCls : 'cancer',
			handler : function() {
				win.hide();
			}
	...
	function showRecord() {
		if (grid.selModel.hasSelection()) {
			...
			if (records.length > 1) {
				...
				url = "clientAppraiseRecordSelect.jsp?cliendId=" + cliendId
...
//					var flag = store.getAt(i).get('gatherFlag');
//					if (flag == 'Y') {
//						Ext.get('btnSave').dom.style.display = "none";//隐藏按钮
//					} else {
////						Ext.get('btnSave').dom.style.display = "";
//					}
//				}
				// modify By ywliu	2009/06/24
				var flag = record.get('gatherFlag');
				if (flag == 'Y') {
					Ext.get('btnSave').dom.style.display = "none";//隐藏按钮
					Ext.get('cancer').dom.style.display = "none";
				} else {
					Ext.get('btnSave').dom.style.display = "";
					Ext.get('cancer').dom.style.display = "";
				}
