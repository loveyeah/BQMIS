	...
	function query() {
		if (dateType != 2) {
			if (reportNameBox.getValue() == "" || startTime.getValue() == ""
					|| endTime.getValue() == "") {
				Ext.Msg.alert('提示信息', '运行报表名称，时段必填');
				return;
			};
			...
			/*// 判断时间间隔不能大于3天
			if (startTime.getValue().substr(0, 7) != endTime.getValue().substr(
					0, 7)) {
				Ext.Msg.alert('提示', '时段的年份和月份必须相同！');
				return;
			}*/ /*else if (endTime.getValue().substr(8, 2)
					- startTime.getValue().substr(8, 2) > 3) {
				Ext.Msg.alert('提示', '时段间隔时间不能大于三天！');
				return;
			}*/
			// else if (parseInt(endTime.getValue().substr(8, 2))
			// - parseInt(startTime.getValue().substr(8, 2)) > 3) {
			// Ext.Msg.alert('提示', '时段间隔时间不能大于三天！');
			// return;
			// }
...
//		alert(reportType)
	...
	reportNameStore.on('load', function(e, records) {
				...
				// add by liuyi 20100513 去掉班组指标的报表
