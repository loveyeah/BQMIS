...
Ext.onReady(function() {
	...
	function modifyBtn() {
		...
		for (i = 0; i < recs1.length; i++) {
			if (recs1[i].get("baseInfo.id.itemCode") == recR.data.itemCode) {
				Ext.Msg.alert("提示", "该台帐已有此指标！");
				return;
			}
		}
	...
	deptTree.on('beforeload', function(node) {
		deptTree.loader.dataUrl = 'manager/findStatTree.action?pid=' + node.id;
	});
	...
	deptTree.on('click', function(node, e) {
		...
		var recs1 = Oldstore.getRange(0, Oldstore.getTotalCount());
		for (i = 0; i < recs1.length; i++) {
			if (recs1[i].get("baseInfo.id.itemCode") == node.id) {
				Ext.Msg.alert("提示", "该台帐已有此指标！");
				return;
			}
