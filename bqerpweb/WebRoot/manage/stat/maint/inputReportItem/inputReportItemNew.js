...
Ext.onReady(function() {
	...
	var MyRecord = Ext.data.Record.create([{
				...
				name : 'model.itemSecondName'
			}, {
				name : 'model.dataTimeType'
			}, {
				name : 'model.conkersNo'
			}
			// add by liuyi 20100524 0值是否显示 Y:显示 N:不显示 默认为显示
			,{
				name : 'model.isShowZero'
			}
			]);

	var dataProxy = new Ext.data.HttpProxy({
				url : 'manager/findReprotItemNewList.action '
			});
	...
	var grid = new Ext.grid.EditorGridPanel({
				...
				columns : [sm, number, {
							...
							renderer : function changeIt(val) {
								for (i = 0; i < dataTimeTypeStore.getCount(); i++) {
									if (dataTimeTypeStore.getAt(i).get("key") == val)
										return dataTimeTypeStore.getAt(i)
												.get("value");
								}
							}
							...
							header : "一级名称排序",
							sortable : false,
							dataIndex : 'model.itemSecondName',
							width : 90,
							editor : new Ext.form.TextField({})
	...
	var con_ds = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'manager/getBpCReportNewList.action' // 运行报表指标名称
						}),
	...
	var con_item_cm = new Ext.grid.ColumnModel([{
				...
				align : 'left',
				width : 240,// modify by ywliu 20090911
				renderer : function(value, metadata, record) {
					metadata.attr = 'style="white-space:normal;"';
					return value;
				}
	...
	function addRecord() {
		...
		if (recL) {
			var args = {
				reportId : recL.get("reportCode"),
				reportType : recL.get("reportType")
				// accountType : recL.get("reportType"),
				// accountCode : recL.get("reportCode")
			}
			// var url = "selectItem.jsp";
			// var url =
			// "../../../../manage/stat/maint/smallItem/reportItem/selectItem.jsp";
			var url = "selectItemNew.jsp";
			var rvo = window
					.showModalDialog(
							url,
							args,
			...
			if (typeof(rvo) != "undefined" && rvo !=null) {
				var i;
				for (i = 0; i < rvo.length; i++) {
					...
					var checkSame = true;
					// for (var j = 0; j < store.getCount(); j++) {
					// if (store.getAt(j).get("model.id.itemCode") ==
					// rvo[i].itemCode) {
					// checkSame = false;
					// break;
					// }
					...
					if (checkSame) {
						var o = new MyRecord({
									...
									'model.itemCode' : '',
									'model.displayNo' : '',
									'model.itemSecondName' : '',
									'itemName' : '',
									'model.itemAlias' : '',
									'model.dataType' : '0',
									'model.dataTimeType' : '1',
									'model.conkersNo' : ''
									// add by liuyi 20100524 
									,'model.isShowZero' : ''
						...
						o.set("model.reportCode", recL.get("reportCode"));
						o.set("model.itemCode", rvo[i].itemCode);
						o.set("model.displayNo", rvo[i].displayNo);
						o.set("model.itemSecondName", "");
						o.set("model.conkersNo", "");
	...
	function deleteRecords() {
		...
		if (selections.length == 0) {
			...
			reportcode = selections[0].get("model.reportCode");
			for (var i = 0; i < selections.length; i += 1) {
				...
				if (member.get("model.id") != null
						&& member.get("model.itemCode") != null
						&& member.get("model.reportCode") != null) {
					id.push("'" + member.get("model.id") + "'");
	...
	function saveModifies() {
		...
		if (modifyRec.length > 0 || ids.length > 0) {
			...
			// alert(ids.join(";"));
			Ext.Ajax.request({
						url : 'manager/addReprotItemNew.action',
						method : 'post',
						params : {
